...
const CIRCUIT_ID = "blog-article-v1";
const SCHEMA = "blog-article-v1";

const WASM_PATH = path.join(PKG_ROOT, "circuits", "build", "blog-article_js", "blog-article.wasm");
const ZKEY_PATH = path.join(PKG_ROOT, "circuits", "build", "blog-article_final.zkey");
...
const uploadToPinata = (filePath: string, fileName: string): Promise<string> => {
  ...
  formData.append("pinataMetadata", JSON.stringify({
    name: fileName,
    keyvalues: { project: "lemma-blog-article", circuit: CIRCUIT_ID },
  }));
...
const buildCircuitMeta = (wasmUrl: string, zkeyUrl: string): CircuitMeta => ({
  ...
  schema: SCHEMA,
  description: "Blog article authenticity — proves knowledge of article attributes (author, published date, content integrity, word count, language) without revealing them.",
...
const requireEnv = (): Promise<void> =>
  LEMMA_API_KEY && PINATA_API_KEY && PINATA_SECRET_API_KEY
    ? Promise.resolve()
    : Promise.reject(new Error("Missing env vars. Set LEMMA_API_KEY, PINATA_API_KEY, PINATA_SECRET_API_KEY in packages/blog-article/.env"));

const requireArtifacts = (): Promise<void> =>
  fs.existsSync(WASM_PATH) && fs.existsSync(ZKEY_PATH)
    ? Promise.resolve()
    : Promise.reject(new Error("Compiled circuit not found. Run `npm run build` in packages/blog-article/circuits first."));
...
const main = async (): Promise<void> => {
  console.log("📝 Registering blog-article-v1 circuit...");
  await requireEnv();
  await requireArtifacts();

  console.log("1. Uploading artifacts to IPFS (Pinata)...");
  const [wasmUrl, zkeyUrl] = await Promise.all([
    uploadToPinata(WASM_PATH, "blog-article.wasm"),
    uploadToPinata(ZKEY_PATH, "blog-article_final.zkey"),
  ]);
