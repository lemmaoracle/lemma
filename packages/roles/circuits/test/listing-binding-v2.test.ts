...
const calculateWitness = async (
  ...
  const wtnsPath = path.join(BUILD_DIR, "test-witness-v2.wtns");
...
const generateProof = async (input: CircuitSignals) => {
  ...
  const zkeyPath = path.join(BUILD_DIR, "listing-binding-v2_final.zkey");
  ...
  const vkeyPath = path.join(BUILD_DIR, "listing-binding-v2_vkey.json");
...
describe.runIf(circuitBuilt)("listing-binding-v2 circuit build artifacts", () => {
  it("R1CS file exists", () => {
    expect(
      fs.existsSync(path.join(BUILD_DIR, "listing-binding-v2.r1cs")),
    ).toBe(true);
  });
  ...
  it("zkey file exists", () => {
    expect(
      fs.existsSync(path.join(BUILD_DIR, "listing-binding-v2_final.zkey")),
    ).toBe(true);
  });
...
describe.runIf(circuitBuilt)(
  "listing-binding-v2 circuit constraints",
  () => {
...
describe.runIf(!circuitBuilt)("listing-binding-v2 circuit (not built)", () => {
