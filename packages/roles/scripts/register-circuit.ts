#!/usr/bin/env node
/**
 * Register listing-binding-v1 circuit with Lemma workers API.
 *
...
const buildCircuitMeta = (wasmUrl: string, zkeyUrl: string): CircuitMeta => ({
  ...
  schema: SCHEMA,
  description: "Trust402 listing binding — thin routing layer proving that schema, content commitment, price, and seller DID are bound into a single listingRoot. Composes with per-schema circuits (blog-article-v1, content-commitment-v1).",
  inputs: ["listingRoot", "perSchemaCommitment", "schemaId", "priceUsdc"],
...
const main = async (): Promise<void> => {
  console.log("🔗 Registering listing-binding-v1 circuit...");
  await requireEnv();
