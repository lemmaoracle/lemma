#!/usr/bin/env node
/**
 * Register listing-binding-v2 circuit with Lemma workers API.
 *
 * Pipeline: Pinata upload (WASM + zkey) → Lemma circuits.register
 *
 * Usage:  npx tsx scripts/register-listing-binding-v2.ts
 *
...
const CIRCUIT_ID = "listing-binding-v2";
...
const buildCircuitMeta = (wasmUrl: string, zkeyUrl: string): CircuitMeta => ({
  ...
  schema: SCHEMA,
  description:
    "Trust402 listing binding v2 — proves listing integrity (Poseidon5 listingRoot) and institutional membership (Poseidon2 Merkle inclusion) in a single proof. Composes with per-schema content circuits; commitment is a public input only.",
  inputs: [
    "listingRoot",
    "commitment",
    "orgDid",
...
const main = async (): Promise<void> => {
  console.log("🔗 Registering listing-binding-v2 circuit...");
  await requireEnv();
