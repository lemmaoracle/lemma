...
describe("commitments", () => {
  ...
  describe("commitNormalized", () => {
    it("rejects empty or non-object normalized input", async () => {
      await expect(commitNormalized({} as Json)).rejects.toThrow(
        "non-empty object",
      );
      await expect(commitNormalized([] as Json)).rejects.toThrow(
        "non-empty object",
      );
      await expect(commitNormalized("string" as Json)).rejects.toThrow(
        "non-empty object",
      );
    });
    ...
    it("produces deterministic leaves for same input", async () => {
      ...
      expect(result1.perAttributeCommitments[0]).not.toBe(
        result2.perAttributeCommitments[0],
      );
    });
    ...
    it("produces different roots when any field changes", async () => {
      ...
      expect(resultBase.attrCommitmentRoot).not.toBe(
        resultModified.attrCommitmentRoot,
      );
    });
    ...
    it("produces different roots with different randomness (hiding property)", async () => {
      ...
      const results = await Promise.all(
        R.times(() => commitNormalized(normalized), 3),
      );
