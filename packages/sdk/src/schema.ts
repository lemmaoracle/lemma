...
export const define = async <Raw, Norm>(schemaMeta: SchemaMeta): Promise<SchemaDef<Raw, Norm>> => {
  ...
  // 4. Load wasm-bindgen JS shim and instantiate WASM module.
  //    The JS shim (generated by `wasm-pack build --target web`) contains
  //    an `init()` default export that internally calls
  //    `WebAssembly.instantiate(buffer, imports)` with the correct `imports.wbg`
  //    namespace, handling all pointer/length ↔ string conversions.
  const resolvedJsUrl = resolveArtifactUrl(artifact.artifact.js);
  const shim = await import(/* @vite-ignore */ resolvedJsUrl);
