...
describe("define / getSchemaById", () => {
  ...
  it("registers and retrieves a schema with WASM normalize", async () => {
    ...
    (crypto.subtle.digest as any).mockResolvedValue(
      new Uint8Array(
        mockHashHex.match(/.{1,2}/g)!.map(byte => parseInt(byte, 16))
      ).buffer,
    );
    ...
    expect(retrieved!.normalize(raw)).toEqual({ weather_bucket: "wet", temperature_bucket: "cold" });
  });
  ...
  it("rejects on missing normalize export", async () => {
    ...
    await expect(define(schemaMeta)).rejects.toThrow("WASM module does not export a 'normalize' function");
  });
