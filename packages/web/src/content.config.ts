...
const briefSchema = z
  .object({
     ...
     *
     * 既存 Brief の値はそのまま残してあるので optional のまま置く。新しい
    ...
    /**
     * ヒーロー直下に置く一行（2026-08 新設）。事案の落差を、読む前に一目で
     * 掴ませるための要約。設定した Brief だけに出る（未設定はヒーロー不変）。
     * ロケールごとに書く。
     *
     * 禁則（Brief 124 の第三者レビューと 4 名のブラインド判定で確定）:
     *   - **反実仮想を書かない**。「◯◯していれば防げた」「検出できた」は、
     *     実在の進行中事案について立証不能な因果を断定することになる。
     *   - **Lemma を検出する主体として書かない**。Lemma は AI の誤りを
     *     検知する製品ではない（各 Brief の §5 が明示している）。ここで
     *     検出を主張すると、記事が自分で自分を否定する。
     *   - 事案の記述に徹し、効果や性能を主張しない。この欄は記事の一部で
     *     あって製品の訴求ではない。
     */
