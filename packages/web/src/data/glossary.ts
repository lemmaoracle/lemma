...
export const GLOSSARY_TERMS: ReadonlyArray<GlossaryTerm> = [
  ...
  },
  {
    ...
    lead:
      "データ・モデル・判断がいつ・誰によって・何を入力として生成されたかを、改ざん不能に追跡・検証する仕組み。検証可能 AI の入力層であり、Lemma の中核をなす証明の一つ。",
    definition: [
  ...
  },
  {
    ...
    definition: [
      ...
      '<a href="/ja/glossary/eu-ai-act/">EU AI Act</a> 第 14 条は高リスク AI に「人間による監督 (human oversight)」を義務付けており、HITL はその最も保守的な実装にあたる。人間が各判断に介在することで、自動化バイアスや誤作動を実行前に遮断できる。',
      "ただし HITL は本質的にスケールしない。判断件数が人間のレビュー能力を超えた時点で、承認待ちのキューが処理速度の上限になる。エージェントが連続的に意思決定する用途では、すべての判断に人間を挟むことが現実的でなくなる。",
    ],
    implementation: [
      'Lemma は「人間が承認した」という事実そのものを <a href="/ja/glossary/audit-trail/">監査トレイル</a> の一要素として記録する。承認者・承認時刻・対象 docHash をコミットメント化し、後から「この判断は人間承認を経た」ことを改ざん不能に証明できる。',
      'これにより HITL は「運用ログ上の主張」から「暗号的に検証可能な事実」へ移る。監査側は原データを見ずに、<a href="/ja/glossary/zk-proof/">ゼロ知識証明</a> で承認の有無だけを確認できる。',
      'HITL を完全に省くのではなく、承認の事実を証跡化したうえで <a href="/ja/glossary/human-off-the-loop/">ヒューマン・オフ・ザ・ループ</a> へ段階的に移行する経路を、同じ監査トレイル設計の上で提供する。',
    ],
    related: [
      { slug: "human-off-the-loop", desc: "人間承認を外し、証跡で代替する自律運用。HITL の対極。" },
      { slug: "audit-trail", desc: "人間承認の有無を記録する改ざん不能なログ。" },
      { slug: "eu-ai-act", desc: "高リスク AI に人間監督を義務付ける第 14 条。" },
      { slug: "verifiable-ai", desc: "人間監督の事実を検証可能要素として含む上位概念。" },
    ],
    ctaH2: "人間承認を、検証可能な事実として残す。",
  },
  {
    ...
    lead:
      "AI が一件ごとの人間承認を待たずに判断を実行する運用形態。人間は必要に応じて監視・介入するが、各判断のゲートには立たない。スループットは飛躍的に上がる一方、人間の目が担っていた説明責任を別の仕組みで埋める必要がある。",
    definition: [
      ...
      '<a href="/ja/glossary/eu-ai-act/">EU AI Act</a> の人間監督要件は HOTL を禁じてはいない。求めているのは「監督できる状態」であり、人間が各判断に立ち会うこと自体ではない。事後に検証可能な証跡があれば、監督の要件は技術側で満たせる。',
    ],
    implementation: [
      'Lemma の立場は明確である。loop を離れた判断ほど、人間承認の代わりとなる <a href="/ja/glossary/audit-trail/">監査トレイル</a> が説明責任を担う。各判断の入力ハッシュ・モデルバージョン・推論時刻・出力ハッシュをコミットメントチェーンに固定し、人間の署名がなくても「いつ・どのモデルが・どの入力で判断したか」を改ざん不能に残す。',
      '<a href="/ja/glossary/agentic-payments/">エージェント決済</a> のように人間が一件ずつ承認できない領域では、<a href="/ja/glossary/zk-proof/">ゼロ知識証明</a> による <a href="/ja/glossary/provenance/">来歴</a> の証明が、人間承認の機能的な代替になる。データを開示せずに、判断の正当性だけを検証できる。',
      "結果として HOTL は「統制を捨てた自律」ではなく「証跡で統制された自律」になる。Lemma が用意するのは、自律度を上げながら監査可能性を落とさないための検証層である。",
    ],
    related: [
      { slug: "human-in-the-loop", desc: "各判断に人間承認を挟む統制モデル。HOTL の対極。" },
      { slug: "audit-trail", desc: "人間承認を外した後、説明責任を担う改ざん不能な証跡。" },
      { slug: "agentic-payments", desc: "人間が一件ずつ承認できない、HOTL が前提の典型領域。" },
      { slug: "provenance", desc: "判断の根拠を事後に再構成するための来歴チェーン。" },
    ],
  ...
  },
  {
    ...
    category: "規制・コンプライアンス",
    description:
      "個人を特定できる、または特定可能な情報の総称。氏名・連絡先・ID 番号・生体情報など。GDPR・改正個人情報保護法・CCPA は「収集の最小化」と「保管漏洩リスクの削減」を事業者に要請する。",
    lead:
      "個人を特定できる、または特定可能な情報の総称。氏名・連絡先・ID 番号・生体情報・端末識別子・位置情報など。GDPR・改正個人情報保護法・CCPA は事業者に「収集の最小化」と「保管中の漏洩リスクの削減」を同時に要請する。",
    definition: [
      "包含範囲は法域ごとに異なる。GDPR の <strong>personal data</strong>、CCPA の <strong>personal information</strong>、日本の改正個人情報保護法の「個人情報」など、定義は微妙に揃わない。共通して扱われるのは、氏名・住所・連絡先・ID 番号(マイナンバー・PPID・PAN 等)・生体情報・端末識別子・位置情報・通信履歴・購買履歴。EU では特別カテゴリ(sensitive PII。医療情報・人種・宗教・生体・性的指向等)に追加保護が要求される。",
      "規制が事業者に課す負荷は二重構造になっている。<strong>収集の必要最小限</strong> と <strong>保管中の保護</strong> を同時に求めるが、両者は本来トレードオフ。後者を強化しても、原データを保管している以上、漏洩面そのものは消えない。事業者にとっての構造的リスクは、保管中の原 PII が永久に攻撃面・規制リスクであり続けること。",
    ...
    implementation: [
      '事業者が原 PII を保管せずに、必要な属性だけを証明として渡せる構造に組み替える。発行者が原 PII に署名 → ホルダーが述語(「18 歳以上」「KYC 通過」「日本居住」など)のみを <a href="/ja/glossary/selective-disclosure/">選択的開示</a> → 検証者は属性のみ確認、という三層で構成される。',
      '原 PII は発行者の管理下に残り、検証者・受信者・AI 推論側のいずれにも触れない。<a href="/ja/glossary/aes-gcm/">AES-GCM</a> 下で暗号化された状態のまま保管され、回路に乗るのは <a href="/ja/glossary/doc-hash/">docHash</a> と属性 <a href="/ja/glossary/commitment/">コミットメント</a> のみ。漏洩した場合の被害面は構造的に縮小される。',
      'この構造は <a href="/ja/glossary/kyc-aml/">KYC/AML</a>・年齢確認・国籍検証など、規制が「属性確認」と「データ最小化」を同時に要請する領域で最も効く。<a href="/ja/glossary/eu-ai-act/">EU AI Act</a> 高リスク AI の入力データ、GDPR 8 条(子供への情報サービス)、改正個人情報保護法の「仮名加工情報」の代替設計としても応用される。',
    ],
    related: [
      { slug: "kyc-aml", desc: "原 PII を渡さずに規制属性を満たす代表領域。" },
      { slug: "selective-disclosure", desc: "PII を秘匿したまま、必要な属性のみ証明する手法。" },
      { slug: "audit-trail", desc: "「誰が・どの属性を・いつ確認したか」を改ざん不能に残す経路。" },
      { slug: "provenance", desc: "PII を扱う処理の来歴を独立検証可能に保つ手法。" },
    ],
    ctaH2: "原 PII を渡さずに、規制属性を満たす。",
  },
];
