 ...
 * レンダリング時に消費されるので、出力側で落とせば壊さずに済む。markdown 由来の
 * コメントや、今後書かれるコメントも自動的に対象になる。
 *
 * **取りこぼす場合がある**: コメントが `<script>`/`<pre>` 等の**マークアップを
 * 中に含む**とき（コメントアウトされたブロック）、保護ブロックの正規表現が
 * コメント内のタグに先に当たり、そのコメントは残る。閉じタグの無い `<script`
 * がコメント内にあると、次の実在の `</script>` までが保護扱いになって間の
 * コメントも残る。**壊れる方には倒れない**（消し損なうだけ）ので、残ったら
 * ソース側を `{/* *\/}` に直す。現状の dist では残存0。
 *
 * 保護するもの:
