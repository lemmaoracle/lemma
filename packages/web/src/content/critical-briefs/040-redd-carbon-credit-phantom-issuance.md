...
2025 年 10 月、ブラジル連邦警察は「Operation Greenwashing」として、Amazon の REDD+ カーボンクレジット事業に関わる詐欺と土地収奪の容疑で 31 名を起訴した。対象は Amazonas 州 Lábrea にある 2 つの REDD+ プロジェクト（Unitor・Fortaleza Ituxi、合計 14 万ヘクタール超）。これらは「森林を保全し炭素を固定した」という環境属性を根拠にクレジットを生成・販売していたが、連邦警察は、同じ区域が他所の違法伐採木材をロンダリングする経路としても使われていたと結論づけた。クレジットは Nestlé・Toshiba・Spotify・Boeing・PwC などの大手企業に数百万ドル規模で販売されていた。乖離を最初に可視化したのは、Mongabay の調査報道（2024 年 5 月）と、その依頼で Center for Climate Crime Analysis（CCCA）が行った衛星画像分析だった。CCCA は、申告された伐採量と衛星から推定される伐採量の不一致を指摘した。本 Brief は、「一定の森林が保全され炭素が固定された」という環境属性が、その根拠データ（保全面積・土地登録・伐採量）の来歴・真正性の独立検証から切り離されたまま、クレジットとして発行され、自主的炭素市場と企業の排出量開示に流入する構造を扱う。本 Brief は、国際的に報道・調査された海外事案を、規制属性の独立検証という観点から構造的に分析する。認証機関・検証会社・検出側の役割は、否定ではなく役割分担として描く。
...
現状、自主的炭素市場の運用モデル全体において、環境属性の独立検証は、事業者から申告される登録データへの信頼に依存しており、まだ独立した層として扱われていない。事前証明（pre-execution attestation）は、発行・取引の経路に属性証明を 1 段挟むことで、この落差を埋める。事前証明は検出に対する代替ではなく **補完** であり、両層の組み合わせでクレジットの trust boundary が確立される（検出と事前証明の関係についての詳細は [「AI 時代のサイバー防衛に残された、最後の層」](https://lemma.frame00.com/ja/blog/detection-is-not-proof/)（Lemma、2026-05）を参照）。
...
これにより、発行・取引の時点で固定された証明が、「当時、その区域は実際に保全されていたか」を後年に問われた際に、原データを開示せず独立検証可能なトレイルとして機能する。検出（事後の衛星監視・調査報道）は発覚後の是正に、事前証明（発行時点の属性検証）は環境属性の独立検証に、それぞれ相補的に働く。設計と適用範囲は、[Pillar 04 — 規制属性証明](https://lemma.frame00.com/ja/pillars/regulatory-attribute-proof/) および [ユースケース一覧](https://lemma.frame00.com/ja/solutions/use-cases/) を参照のこと。
...
[Discovery Call →](https://tally.so/r/EkBqDX)　[ホワイトペーパー →](https://tally.so/r/xX0VYv)　[✉️ ニュースレター →](https://tally.so/r/EkMj82?ref=brief-cta)
