...
2026 年、セキュリティ業界団体 CSA（Cloud Security Alliance）が、AI エージェントの「スキル」やツール定義に、人の目には見えない文字を仕込んで AI を操る攻撃手法を開示した。一部の Unicode 文字は、多くのエディタや画面では空白として、または何も表示されないが、AI（言語モデル）は意味のある指示として読み取ってしまう。攻撃者は、悪意ある指示をこの見えない文字に変換し、スキルファイル・文書・ツール定義の中の——見出しや行末や空白に紛れ込ませ——人間のレビューでは気づけない形で埋め込める。関連する供給網の監査（Snyk ToxicSkills、2026-02）では、3,984 件のスキルのうち 36.82% に何らかのセキュリティ上の問題、13.4% に重大な問題があり、76 件の悪意あるペイロードが確認された。つまり「人が画面で見た内容」と「AI が実際に読んだ内容」が一致する保証は、検証層なしには存在しない。本 Brief は、AI に渡る入力の出所と中身の同一性が独立検証されないとき、人間のレビューが安全性の保証にならない構造を扱う。
