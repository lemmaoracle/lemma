...
協働ロボット Universal Robots の制御ソフト PolyScope 5（5.25.1 より前）に、未認証の攻撃者がロボットの OS で任意コードを実行できる弱点（CVE-2026-8153）があると CISA が警告した。制御ソフトが外部入力を確かめず OS に渡し、ネットワーク到達だけで操作できた。CISA 勧告・パッチ・分離では、この命令の送り手がロボットを操作する正当な権限を持つかを物理動作の前に確かめられない。命令の認可を検証する層そのものが経路上に存在しなかった。検出と、行動前に認可を独立検証する事前証明は代替でなく補完である。
