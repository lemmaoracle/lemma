...
米政府機関 CISA が、世界で広く使われる協働ロボット Universal Robots の制御ソフト PolyScope 5（5.25.1 より前）に、深刻な弱点（CVE-2026-8153、最高水準の深刻度）があると警告した。制御ソフトが、外部から届いた入力を確かめずにロボットの OS にそのまま渡していたため、ロボットの操作権限を一切持たない相手でも、ネットワークさえつながればロボット上で任意のプログラムを実行でき、制御の改ざんや運転妨害につながりうる。つまり「正当な操作者か」をロボットが動く前に確かめる仕組みがなく、ネットワークに届くこと自体が操作できることになっていた。メーカーは 5.25.1 で修正済みで、本稿時点で実環境での悪用は報告されていない。
