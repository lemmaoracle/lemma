...
セキュリティ企業 Aim Labs（Aim Security）が 2025 年 6 月 11 日に公表した EchoLeak（CVE-2025-32711, CVSS 9.3）は、Microsoft 365 Copilot に細工したメールを 1 通送るだけで、利用者が何も操作しなくても（ゼロクリック）、Copilot がアクセスできる範囲（OneDrive・SharePoint・Teams・チャット履歴など）の社内の機微データを社外の攻撃者のサーバへ送り出させられるものだった。メールに紛れ込ませた「命令」を、AI が処理すべきデータと区別できなかったためである。AI が動く瞬間に、その動作の指示が「どこから来た、正当なものか」を確かめる層が無かったことが核心で、Microsoft はサーバ側で修正済み・実環境での悪用証拠はないとしている。
