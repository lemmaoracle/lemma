...
行動時点で固定された証明は、後日「このアクセスは正当な権限に基づいていたか」を問われたときに、個人データを開示せずに独立検証できる証跡として機能する。検出・開示（研究者調査・修正）は被害の縮小に、事前証明（アクセス時の権限検証）は認可の独立検証に、それぞれ補完的に寄与する。設計と適用範囲は、[Pillar 03 — エージェント権限証明](https://lemma.frame00.com/ja/pillars/agent-authority-proof/) を参照のこと。
