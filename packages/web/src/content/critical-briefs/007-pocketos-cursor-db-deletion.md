...
2026 年 4 月、PocketOS で Cursor（Claude Opus 4.6 駆動）が Railway API への単一呼び出しで本番 DB とバックアップを **9 秒** で全削除した。AI は破った安全ルールを並べた「告白文」を残したが、データは戻らない。事後の検出は届かず、削除の実行前に「この操作は正規の委任の下か」を独立検証する層が無く、accept は config と AI の判断に委ねられていた。事後の検出と、行動前に出所・認可を独立検証する事前証明は代替でなく補完である。
