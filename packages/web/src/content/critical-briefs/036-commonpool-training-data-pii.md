...
研究チームが、最大級の公開 AI 学習データセット DataComp CommonPool（2023 年公開、128 億の画像とテキストの組）に、パスポート・クレジットカード・出生証明書・運転免許・履歴書など、実在する個人の個人情報が大量に含まれていることを報告した。監査したのは全体のわずか 0.1% だが、そこから数千件の有効な ID 書類と、800 件超の実在が確認できる応募書類が見つかり、全体では数億規模の個人画像があると推定された。運営側は顔の自動ぼかし等の対策をしていたが、標本中だけで 800 超の顔が漏れ（全体では約 1 億 200 万と推定）、メールや社会保障番号のフィルタもかかっていなかった。このデータセットは Stable Diffusion・Midjourney 等の学習系譜に連なるため、下流のモデルへの個人情報の波及も高い蓋然性で起きる。
