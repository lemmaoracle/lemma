...
AIモデルへのアクセスを一元化する「AIゲートウェイ」LiteLLM(BerriAI開発、オープンソース)のMCP(Model Context Protocol)Streamable HTTPエンドポイントに、認証バイパスの脆弱性CVE-2026-59822が存在した。LiteLLM自身のAPIキー検証が失敗した場合のフォールバック処理が、その失敗という結果を「空の(=常に有効とみなされる)認可オブジェクト」に置き換える設計になっており、偽造したAuthorizationヘッダを送るだけで、有効なLiteLLMキーを一切持たない攻撃者が、接続先のMCPツール・サービスに到達できた。GitHubは2026年6月30日にこの脆弱性を公表しv1.84.0で修正したが、CISAは2026年9月2日、実際の悪用を確認したとしてKEV(既知の悪用された脆弱性)カタログに追加し、連邦機関に9月16日までの是正を義務づけた。**認証は「失敗する」ようには作られていたが、失敗した先で「拒否する」層はなく、検証されていないことがそのまま許可されたこととして扱われた。**
...
- 修正版の公開から約2か月後の2026-09-02、CISAはこの脆弱性の実際の悪用を確認したとして、KEV(Known Exploited Vulnerabilities)カタログに追加した。同日追加された7件のうち3件がAI関連基盤の脆弱性(本件LiteLLMに加え、Starlette[CVE-2026-48710、[Lemma Critical Brief 003](https://lemma.frame00.com/ja/critical/briefs/003-starlette-badhost/)で既出]、JFrog Artifactory)であり、セキュリティ研究者からは「KEV追加バッチでAI基盤の脆弱性がまとまった数を占めた最初の例」との指摘が出ている。
...
<strong>「失敗した検証」と「検証をしていないこと」が、システムの振る舞いとしては同じ結果を生んだ。</strong>これは特定の巧妙な回避技術ではなく、フォールバック処理の設計そのものの問題であり、この設計は本番稼働のまま2か月以上——GHSAでの公表(06-30)からCISAが実悪用を確認しKEVへ追加する(09-02)まで——動き続けていた。
...
事後の脆弱性スキャン・KEV登録との違いはここにある。CISAのKEV追加は、脆弱性が実際に悪用されたことを事後に確認し公表する仕組みであり、GHSA公表からKEV追加までの約2か月間、この設計は本番のMCPゲートウェイの中で動き続けていた。
