...
2026 年 6 月、研究者 RyotaK（GMO Flatt Security）は、Claude Code GitHub Action が起動者の名前末尾 `[bot]` を無条件に信頼するため、偽装起動とプロンプトインジェクションで悪意ある issue 1 件からリポジトリを乗っ取れる欠陥を公開した。4 日で修正されたが、事後の検出・パッチは起動者の権限や入力の出所を実行の前には立証しない。欠けているのは、特権実行の前に起動者の権限と入力の出所を独立検証する層である。検出と事前証明は代替でなく補完である。
...
- **核心**: エージェントが特権実行する前に、起動者の権限と入力の出所を独立検証する層が無く、`[bot]` という名前と依頼を装った issue 本文が、権限の証明と信頼できる入力として素通りした
