...
開発チームが CI に AI を組み込み、寄せられた依頼（issue）を AI に仕分けさせるのは、いまや普通の運用だ。だが、その AI が「依頼してきた相手を信頼してよいか」を確かめずに書かれた内容を指示として実行すると、依頼 1 件でリポジトリ全体を乗っ取られる。2026 年 6 月、あるセキュリティ研究者（GMO Flatt Security）は、Anthropic の Claude Code GitHub Action に、悪意ある依頼 1 件で脆弱なリポジトリを乗っ取れる欠陥を公開した。AI を起動する判定が、名前が `[bot]` で終わる相手を無条件に信頼していたため、誰でも偽装した起動者で AI を呼び出し、依頼文に紛れ込ませた指示で社内の認証情報を引き出させ、最終的にリポジトリへの書き込み権限まで奪える経路が成立しえた。Anthropic は報告から 4 日で修正し、研究者に報奨金を支払った。なお、この経路が実環境の攻撃に使われた痕跡はなく、研究者は自身のテスト環境でのみ実証している。
