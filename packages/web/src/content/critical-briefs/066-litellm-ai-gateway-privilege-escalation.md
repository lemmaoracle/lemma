...
社内の AI 利用を一か所に束ねる中継層（AI ゲートウェイ）として広く使われる OSS の LiteLLM で、セキュリティ企業 Obsidian Security の検証により、ごく普通の一般ユーザーが管理者権限を奪い、さらにサーバー上で任意のプログラムまで実行できる経路（3 件の弱点を連鎖させると最高レベルの深刻度）が見つかった。権限を確かめる仕組みが何段にも分かれていたのに、どの段も「前の段が確かめたはず」と互いに任せ合い、結局どこも実際には確かめていなかったのが原因である。さらに、乗っ取られたゲートウェイが AI エージェントとモデルの間に割り込み、エージェントに偽の操作を実行させられること（社内の認証情報や各モデルへの API キーにも到達しうる）まで実証された。修正版が公開済みで、この連鎖自体の実環境での悪用は本稿時点で報告されていない。
