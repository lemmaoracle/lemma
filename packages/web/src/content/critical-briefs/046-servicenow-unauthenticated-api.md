...
業務システムに「データを返す API」があるのは普通のことだ。だが 2026 年 6 月、ServiceNow の一部の API が、ログインも認証情報の確認もないまま、顧客のデータを誰でも照会できる状態で提供されていたことが分かった。「ここは必ず認証を求めよ」という設定が一つ外れたまま出荷され、6 月 2〜3 日には外部からの不審なアクセスで実際に顧客データの照会が成功していた（該当 API・観測 IP・是正と通知の時系列は後述）。問題の本質は、API が「認証は設定が守ってくれているはず」という前提に頼り、**要求してきた相手にそのデータを見る資格があるかを、応答の前に確かめていなかった**点にある。
...
本事案で露呈した構造（エンドポイントへの要求が、要求者の認可を行動ごとに証明させるのではなく、設定上の前提に依存して受理される）に対して、Lemma は、認証を「設定が認証を担保しているか」から「要求ごとのスコープされた認可・来歴の応答前証明」へ反転させる設計を提示している。設定状態や長期資格情報に依存せず proof を提示する proof-as-auth の考え方では、エンドポイントの設定がひとつ外れても、正規の認可・来歴の proof が成立しなければ照会は事前に reject される。エンドポイントの応答を「呼び出された」ではなく「この要求は認可・来歴を持つ」という事前証明に紐付けることで、設定ドリフトや未認証要求があっても、認可されていない照会を実行前に区別できる。
