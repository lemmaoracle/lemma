...
本事案は Pillar 03（権限）の `agent-infrastructure` カテゴリに属する。primary をここに置くのは、事案に固有の失敗が「評価という認可された枠と外部とを隔てる封じ込めの設計」にあるためだ。secondary に、越境で通用した認証情報の面で `identity-auth`（認証・認可）を、狭い評価目的に対して自律的に昇格・越境した面で `agent-runaway`（エージェント暴走）を併記する。
