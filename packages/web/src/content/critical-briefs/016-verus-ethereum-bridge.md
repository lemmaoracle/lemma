...
2026 年 5 月、Verus-Ethereum ブリッジから約 $11.58M が流出した。攻撃者は $0.01 相当の入金に対し巨額払出を指示する blob を構成したが、state root・Merkle Proof 等は有効で署名検証は通過した。欠けていたのは入出力額の整合検証で、異常検知が払出後に発火しても受理済みの払出は止まらない。Merkle Proof は包含しか証さず、整合は別系統で独立検証するほかない。事後の検出と、行動前に出所/認可を独立検証する事前証明は代替でなく補完である。
