...
これにより、生成の時点で固定された証明が、「この生成は属性検証・認可・来歴を備えるか」を、拡散の前に独立検証可能なトレイルとして機能させる。検出・削除（事後のモデレーション・規制対応）は被害の除去・抑止に、事前証明（生成時点の属性検証・来歴付与）は有害な生成の未然区別に、それぞれ相補的に働く。設計と適用範囲は、[Pillar 04 — 規制属性証明](https://lemma.frame00.com/ja/pillars/regulatory-attribute-proof/) および [Pillar 01 — 来歴証明](https://lemma.frame00.com/ja/pillars/verifiable-origin/) を参照のこと。
