...
開発者が AI コーディングツール（Claude Code・Cursor・Gemini CLI 等）でリポジトリを「開く」普段の操作が、社内の認証情報を盗み出す引き金になり得ることが、2026 年 5〜6 月に研究と実地の両面から示された。原因は、AI エージェントが、プロジェクトに同梱された設定ファイルを中身も出どころも確かめないまま自動で実行してしまう点にある。実地では、自分自身を増やしながら広がるマルウェアが仕込まれた設定により、開発者がプロジェクトを開いた瞬間に GitHub・クラウド等の認証情報が社外へ吸い出され、2026-06-05 には GitHub が Microsoft 系 4 組織の 73 リポジトリを一斉に無効化した。
