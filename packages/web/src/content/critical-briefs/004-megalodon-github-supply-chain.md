...
Megalodon は、窃取した開発者の正規認証情報を悪用し、6 時間で GitHub 上 5,561 リポジトリに偽装コミットを送り込んだ CI/CD 認証情報窃取キャンペーンである。3 社が 5 日内に起点と範囲を特定したが、検出は受信側が「何を accept するか」を変えない。偽装コミットは正規プロセスを経て push され受理された。commit author と repo owner の認証が各段で独立検証されず chain を成していた点が gap であり、コードを取り込む前に出所を独立検証する事前証明は、事後の検出の代替でなく補完である。
