...
2024 年 2 月、偽造 ID 生成サービス「OnlyFake」の英国パスポート画像で大手暗号資産取引所 OKX の本人確認（KYC）を通過できたと 404 Media が報じた。KYC の審査は身分証画像が本物らしいかを見るだけで、発行元が実際に発行した文書かを確かめない。検出強化は不正試行の多くを止めるが、生成系との軍拡競争であり、提示画像が発行者の発行物かには答えられない。欠けているのは、口座開設前に発行者署名を暗号的に検証する層である。事後の検出と、行動前に出所・認可を独立検証する事前証明は代替でなく補完である。
