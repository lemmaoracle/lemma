...
二つのブロックチェーンを橋渡しする Syscoin のブリッジで、片方で資産を消却した事実が実際には存在しないのに、もう片方で約 50 億 SYS（当時約 850 万ドル相当、一部報道は約 1,000 万ドルと概算）が裏づけなく発行された。暗号方式そのものが破られたわけではない。攻撃者は、消却の事実を確かめるはずの「証明」を本物そっくりに偽造したのではなく、その証明を読み取るプログラムの欠陥を突くように細工した偽の証明を送り込み、システムがそれを「本物の消却の証明」と取り違えて発行を承認してしまった。これは 2022 年の Nomad ブリッジ事件と同型で、証明の中身を取り違える実装上の落差が原因である。発覚後にブリッジは停止され、取引所と連携して資産の凍結・追跡が進められた。
