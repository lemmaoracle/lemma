...
セキュリティ企業 Aikido の検証で、Google の API キーは削除した後も最長で約 23 分間そのまま使えてしまうことが分かった。漏洩に気づいて鍵を削除しても、削除がすべてのサーバーに行き渡るまでの間は攻撃者が使い続けられ、Gemini ではファイルや会話内容の外部送信が可能で、課金面では数百万円規模の被害事例も報告されている。Google は「想定通りの動作で修正しない」との立場である。
