...
2026 年 5 月 22 日、Microsoft Threat Intelligence は、インターネットに面した F5 BIG-IP エッジ機器を起点に、Linux ホスト・内部 Atlassian Confluence・Windows 認証基盤を経てドメイン全体の侵害に至った多段侵入の全攻撃チェーンを公開した。初期侵入は Azure 上の F5 BIG-IP Virtual Edition(VE、バージョン 15.1.201000、2024-12-31 に EOL でパッチ供給停止)への SSH アクセスで、そこから最初の Linux ホストへ横展開した。攻撃が成立した本質は、**エッジ機器が外部露出・低監視でありながら社内で暗黙に高信頼に扱われ、その侵害が保存された資格情報・証明書・ID 統合ごと攻撃者に渡った**点にある。横展開の各ホップは、行動ごとに認可を証明させるのではなく、ネットワーク上の位置と保存された資格情報を信頼して受理された(SMB NTLM reflection の CVE-2025-33073 は、有効なドメイン資格情報さえあれば管理者権限なしに SYSTEM での RCE を成立させた)。本事案は Pillar 03(エージェント権限証明)を「アクター一般の権限・ID の証明」として読む観点における、**信頼が"位置"と"保存資格情報"に基づき、行動ごとに証明されていない**構造を示す。Brief 006(資格情報の状態が独立検証されない)・029(過剰スコープの委任)に連なる。
...
本事案で露呈した構造(横展開の各ホップが、行動ごとの認可証明ではなく位置的信頼と保存資格情報への暗黙信頼で受理される)に対して、Lemma は、認証を「資格情報の保有」から「行動ごとのスコープされた認可・来歴の実行前証明」へ反転させる設計を提示している。鍵や長期資格情報を送らずに proof を提示する proof-as-auth の考え方では、エッジ機器から盗まれた資格情報や reflection で得た権限があっても、正規の認可・来歴の proof が成立しなければ行動は事前に reject される。設計思想は [Pillar 03 — エージェント権限証明](https://lemma.frame00.com/ja/pillars/agent-authority-proof/)(Lemma)および [「Proof-as-Auth: 鍵を一度も送らずにサインインする」](https://lemma.frame00.com/ja/blog/proof-as-auth-sign-in-without-sending-your-key/)(Lemma、2026-05)を参照のこと。
