...
AI エージェントは処理の途中経過を「チェックポイント」（いわばエージェントの記憶）として保存し、後でそこから再開する。2026 年 6 月 12 日、セキュリティ企業 Check Point Research の研究者 Yarden Porat が、LangChain のエージェント基盤 LangGraph で、攻撃者がこの保存済み記憶に偽のデータを紛れ込ませ、エージェントがそれを検証せず読み戻した瞬間にサーバーを乗っ取れる（遠隔コード実行に至る）連鎖した脆弱性を公開した。成立すると、エージェントが握る社内の認証情報（LLM の API 鍵、会話履歴、接続先の CRM や内部 API のアクセス情報）が露出する。エージェントが自分の保存済み状態を「いつ・誰の権限で・改ざんなく書かれたか」を確かめずに信頼して読み戻すことが核心で、3 件とも修正版が出た状態での公開であり、マネージド構成（LangSmith）は影響を受けない。
