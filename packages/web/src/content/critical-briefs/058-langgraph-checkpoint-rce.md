...
- **核心**: エージェントが自分の永続状態（チェックポイント）を、来歴も完全性も行動前に独立検証しないまま特権文脈で読み戻し、注入された偽の状態がそのままコード実行に転化した
