...
AI エージェントが処理の途中経過を「チェックポイント」として保存し、後でそこから再開する——マルチエージェント基盤では当たり前の動作だが、その保存済み状態をエージェントが読み戻すときに、状態が正規のものかを確かめる層がないと、状態そのものが乗っ取りの入口になる。2026 年 6 月 12 日、研究者 Yarden Porat（Check Point Research）が、LangChain のエージェント基盤 LangGraph に 3 件のパッチ済み脆弱性を公開した。中核は、CVE-2025-67644（SQLite チェックポイント実装の SQL インジェクション）と CVE-2026-28277（msgpack の安全でないデシリアライズ）を連鎖させ、自己ホスト構成で遠隔コード実行（RCE）に至るものだ。攻撃者は SQL インジェクションで偽のチェックポイント行をクエリ結果に紛れ込ませ、エージェントがその BLOB をデシリアライズした瞬間に任意コードを実行する。成立すると、エージェントが握る LLM API 鍵・会話履歴・接続先 CRM/内部 API の認証情報が露出する。本事象は Pillar 03（エージェント権限証明）の `agent-infrastructure` における、**エージェントが自分の永続状態を、来歴と完全性を検証しないまま特権的に解釈する**構造を示す事例であり、Brief 027（接続先設定が特権文脈で解釈される）・025（MCP リファレンス SDK の RCE 経路）・003（接続の入口での認証回避）・037（設定の無検証自動実行）に続くエージェント基盤の信頼境界の問題を構成する。
...
事前証明（pre-execution attestation）は、エージェントの状態の読み戻しを権限行為として扱い、状態が再構築される前に「その状態の作者性（どのエージェント・実行が書いたか）」と「完全性（改ざんがないか）」を独立検証可能な証明として要求する設計を採る。proof が「このチェックポイントは認可された実行に由来し、改ざんがない」ことを満たさなければ、状態のロードは実行前に block される。検出と事前証明の関係は [「AI 時代のサイバー防衛に残された、最後の層」](https://lemma.frame00.com/ja/blog/detection-is-not-proof/)（Lemma、2026-05）を参照。
...
本事象で露呈した検出と証明の落差（エージェントが自分の永続状態を、来歴と完全性の検証なしに特権文脈で再構築する）に対して、Lemma は、エージェントの状態の書き込み・読み戻しを権限行為として証跡化し、状態がランタイムに再構築される前に「どの実行が・どの権限で書いたか」「改ざんがないか」を独立検証可能な証明として検証する設計を提示している。エージェント権限証明の設計思想は [Pillar 03 — エージェント権限証明](https://lemma.frame00.com/ja/pillars/agent-authority-proof/) を参照のこと。
