...
AI コーディングツールに「このプロジェクトではこう作業して」と指示する設定ファイル（`.cursorrules` や `CLAUDE.md`）は、いまや開発の日常品だ。だが、その指示を誰が・どんな経緯で置いたかは、AI がそれを読んで従う時点では確かめられていない。2026 年 5 月、セキュリティ企業 Socket は、npm など主要なパッケージ配布元をまたいで開発者の認証情報を盗む TrapDoor というキャンペーンを公表した。固有の手口は、**AI アシスタント宛ての指示ファイルに画面上は見えない命令を埋め込み、AI に「セキュリティスキャン」と称する作業をさせて、開発環境にある社内の認証情報を外部へ持ち出させる**点にある。攻撃者は実在の有名プロジェクトへ、無害を装ってこの指示ファイルを追加する提案まで送っていた（パッケージ・提案・攻撃者文書の詳細は後述）。
