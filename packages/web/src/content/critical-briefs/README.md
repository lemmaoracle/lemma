...
og_lead_en: "<same, EN>"
gap_detected: "<検出は何が効いたか — 1〜2 文>"
gap_missing: "<効かなかった層は何か — 1〜2 文>"
gap_fix: "<Lemma で何が変わるか — 1〜2 文>"
```

- **`gap_*` 3点セットは必須扱い**。右レールの常設パネル「この Brief の核心」
  （✓検出は効いた／✕証明が無かった／→Lemma で変わる）になる。各説明は
  **レール上で4行にクランプされる**ので短く書く。`gap_missing` は TL;DR の
  結び（後述）と呼応させる。
...
（落差の要約は右レールの gap_* パネルが常設で担う）。「検出と証明の落差」
...
- gap_* frontmatter も EN ファイルでは英語で書く（両ファイルの frontmatter は
  title / title_en 等を共有しつつ、gap_* と og_lead_* は各ロケールの言語）。
