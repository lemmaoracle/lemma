...
ある研究チームが Discord の公開 API で 20.52 億メッセージ（3,167 サーバー）を一括収集し、arXiv 論文と JSON dataset として誰でも入手できる形で公開した。だが Discord の規約は、API 取得メッセージの AI 学習利用も一括 scraping・再配布も明確に禁じている。技術的なアクセス可否と規約が許す範囲は別物だが、配布前に「適法 scope で収集されたか」を独立検証する層が無く、禁じられた用途のデータが下流の AI 学習へ流れ得た。事後の検出と、配布前に出所・利用条件を独立検証する事前証明は代替でなく補完である。
