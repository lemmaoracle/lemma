...
暗号資産プロジェクトのマルチシグは、複数の鍵保有者の承認をそろえて初めて資金が動く——権限は分散しているはずだった。だが 2026 年 6 月 8〜9 日、開発者 1 人の業務用ラップトップがマルウェアに感染しただけで、必要数を超える 7 つの秘密鍵が一度に奪われて閾値を単独で突破され、Humanity Protocol から 3,200 万ドル超が流出、H トークンは数時間で 80% 超下落した。鍵を握った者はブリッジからの引き出しに加え、コントラクトを書き換えて 1 億 H 超を新規発行までした。一方、オンチェーン調査者 ZachXBT は自作自演の可能性も指摘しており、外部攻撃か内部かは確定していない（本 Brief は帰属を断定しない）。いずれにせよ、分散承認の各署名が別個の正規主体による意図的な承認だと、実行の前に確かめる仕組みが無かった点は変わらない。
