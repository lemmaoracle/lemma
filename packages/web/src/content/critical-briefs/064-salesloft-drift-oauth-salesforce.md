...
ある SaaS と顧客管理システム（Salesforce）を「連携」させると、その連携アプリは利用者に代わってデータを読み書きできる認証情報を持つ——便利だが、それが盗まれれば攻撃者は利用者のシステムにそのまま入れる。しかも同じ連携が数百社に導入されているため、被害は一社では止まらない。2025 年 8 月、ある攻撃グループは、チャットボット連携 Salesloft Drift が保持していた認証情報を悪用し、700 社を超える Salesforce 環境を横断的に照会した。発端は、攻撃者が事前に Salesloft の開発基盤を侵害し、顧客連携用の認証情報を盗み出していたことにある。攻撃者はとりわけ、サポート案件の本文に貼り付けられたまま放置されていた AWS キーや VPN 認証情報などの社内の認証情報を探し集めた。Salesloft は問題のトークンを失効させ、連携を撤去した。
