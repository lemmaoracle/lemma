...
- **委任スコープの証跡化**: OAuth トークンのような委任権限に、対象リポジトリ・有効範囲を検証可能なスコープとして埋め込み、発行者署名付きで証跡化する
- **特権行為の実行前認可**: 拡張インストールのような特権行為を、実行前に「登録者の認可」と「委任範囲」に照らして独立検証する。委任範囲を超える行為は検出ではなく実行前の拒否として止まる
- **最小スコープの強制**: トークンが過剰に有効でも、proof が「この行為は対象外リポジトリに及ぶ」「このトークンは当該リポジトリ外に有効であってはならない」と告げれば実行前に reject する
