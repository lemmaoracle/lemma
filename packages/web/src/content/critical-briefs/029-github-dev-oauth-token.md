...
2026 年 6 月 2 日、セキュリティ研究者 Ammar Askar が、ブラウザ版 VS Code である github.dev の脆弱性と実証コードを公開した。攻撃者が用意した github.dev リンク(Jupyter Notebook を含むリポジトリ)を 1 クリックさせるだけで、webview 内のスクリプトが**実ユーザーの操作ではなく合成したキーイベントを VS Code 本体へ中継**し、悪性拡張のインストールへ誘導、github.dev が保持する GitHub OAuth トークンを窃取する。決定的なのは、このトークンが**閲覧中のリポジトリにスコープされておらず、ユーザーがアクセスできる全リポジトリに有効**だったこと。実証では窃取トークンで非公開リポジトリ一覧を列挙できた。Microsoft は翌 6 月 3 日に暫定修正(ブラウザ版 Notebook を開く際の信頼確認、拡張インストールコマンドの呼び出し元検証)を投入。本稿執筆時点で CVE 未付番。本事象は Pillar 03(エージェント権限証明)の `agent-infrastructure` における、**権限委任(OAuth トークン)が最小権限にスコープされず、特権行為(拡張インストール)が独立認可なく実行される**構造を示し、Brief 027(MCP 接続設定の特権文脈)・003(MCP server 認証回避)に連なる。
...
事前証明(pre-execution attestation)は、委任トークンに検証可能なスコープ(対象リポジトリ・有効範囲)を埋め込み、拡張インストールのような特権行為を実行前に「登録者の認可」と「委任範囲」に照らして独立検証する設計を採る。proof が「この行為は委任範囲を超える」「このトークンは当該リポジトリ外に有効であってはならない」と告げれば、行為は実行前に block される。特権行為の検出(detection 的な「不審な拡張が動いた」)と委任権限の事前証明(「この行為は認可された範囲内か」)は代替ではなく**補完**の関係にある(検出と事前証明の thesis は [「AI 時代のサイバー防衛に残された、最後の層」](https://lemma.frame00.com/ja/blog/detection-is-not-proof/)(Lemma、2026-05)を参照)。
...
本事象で露呈した検出と証明の落差(委任トークンが最小権限にスコープされず、特権行為が独立認可なく実行される)に対して、Lemma は、エージェント基盤への権限委任・特権行為を証跡化し、実行前に「誰が・何を・どの範囲で」認可したかを独立検証可能な証明として検証する設計を提示している。OAuth トークンが過剰に有効でも、委任範囲の proof が「この行為は対象外リポジトリに及ぶ」と告げれば行為は事前に reject される。エージェント権限証明の設計思想は [Pillar 03 — エージェント権限証明](https://lemma.frame00.com/ja/pillars/agent-authority-proof/)(Lemma)を参照のこと。
