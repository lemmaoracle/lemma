...
リビングのスマートテレビは、たいてい電源が入りっぱなしで、速い回線につながり、使い放題で、誰も画面を見ていない時間が長い——AI 学習用データの収集にとって、これ以上ない中継拠点になる。2026 年 6 月、研究者（Include Security と独立研究者 Buchodi）は、データ事業者 Bright Data が無料アプリに組み込む SDK をリバースエンジニアリングし、その SDK が常時稼働のスマートテレビを含む機器を、**Web スクレイピングのトラフィックを中継する exit node** に変えていることを示した。スクレイピングは Bright Data の顧客の IP ではなく、**家庭の IP** から出ていく。Bright Data は「世界最大級の住宅用プロキシ網（4 億超の住宅 IP を広告）」を運営し、その一部を「同意に基づく 1.5 億超の IP プール」として、無料アプリ内のオプトイン画面の背後で供給する。だが解析によれば、オプトイン画面の表示と SDK が実際に許す挙動は一致せず（ある Roku アプリは「ときどき」と表示しつつ、SDK 設定は月 200GB まで許容）、中継ジョブを運ぶ通信は実質的な認証を欠き、iOS では設定済みの VPN を迂回する。本事案を Pillar 01（来歴証明）の観点から、**AI 学習に向かう収集データと、その中継トラフィックの出所・同意が、収集の時点で独立検証されない**構造として分析する。本 Brief は個々の主体の非難を目的とせず、来歴と同意の検証不在という構造を扱う。Brief 008（公開≠同意）・036（学習データに混入した個人情報）・011（来歴標識）に連なる。
...
事前証明（pre-execution attestation）の発想は、データ収集を「事後に出所を推測する」から「収集の時点で、出所と同意を独立検証可能な証跡にバインドする」へ反転させる。中継トラフィックを、真に同意した機器の証明に紐付け、収集データをその出所・同意の来歴にバインドすれば、来歴・同意の証明を欠くデータは、AI 学習へ取り込む前に検証で弾ける。収集インフラの検出（detection 的な「どの機器が中継しているか」）と来歴の証明（「このデータはどの出所・同意のもとで集められたと独立検証できるか」）は代替ではなく **補完** の関係にある（検出と事前証明の thesis は [「AI 時代のサイバー防衛に残された、最後の層」](https://lemma.frame00.com/ja/blog/detection-is-not-proof/)（Lemma、2026-05）、行為の時点で来歴を独立検証する考え方は [「Proof-as-Auth: 鍵を一度も送らずにサインインする」](https://lemma.frame00.com/ja/blog/proof-as-auth-sign-in-without-sending-your-key/)（Lemma、2026-05）を参照）。
...
これにより、収集の時点で固定された証明が、「この学習データは、検証可能な出所と同意のもとで集められたか」を、事後の遮断・制限に依存せず独立検証可能なトレイルとして機能させる。検出（事後の解析・遮断・プラットフォーム制限）は被害の是正に、事前証明（収集時点の来歴・同意の独立検証）は AI 学習データの信頼確立に、それぞれ相補的に働く。設計と適用範囲は、[Pillar 01 — 来歴証明](https://lemma.frame00.com/ja/pillars/verifiable-origin/) および [ユースケース一覧](https://lemma.frame00.com/ja/solutions/use-cases/) を参照のこと。
