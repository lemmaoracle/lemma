...
開発者が npm パッケージをインストールする普段の操作が、開発環境の社内認証情報をまるごと盗まれ、その認証情報が次の感染の起点になり得ることが、あらためて示された。2026 年 6 月 4 日、セキュリティ企業 JFrog は、自分自身を増やしながら広がる npm マルウェア「IronWorm」（約 37 パッケージに感染）を報告した。このマルウェアはインストール時に自動起動し、クラウド・GitHub の認証情報に加え主要 AI サービスの API キーまで吸い出したうえで、盗んだ認証情報で被害者のリポジトリに自分自身を書き込み、開発者の正規の公開手順を借りて npm に再公開して感染を連鎖させる。盗まれた認証情報が、そのまま「公開してよい権限」として通用してしまうことが核心である。
