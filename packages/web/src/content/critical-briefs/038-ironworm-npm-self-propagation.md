...
JFrog が報告した npm マルウェア「IronWorm」は、開発環境の認証情報を盗み、その鍵で被害者のリポジトリに自身を書き込み正規の公開手順で再公開する自己増殖ワームである。レジストリの無効化やベンダー解析は、公開され鍵が盗まれた後に作動する事後の検出にとどまる。欠けているのは、公開時に「公開者が本当に正規の作者か」を独立検証する層であり、有効な token の提示だけが公開の根拠になっている。事後の検出と、行動前に出所・認可を独立検証する事前証明は代替でなく補完である。
