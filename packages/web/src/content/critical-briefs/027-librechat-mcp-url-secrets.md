...
広く使われる AI チャット基盤 LibreChat（0.8.3 以前）に、深刻度 9.6（10 点満点で最高位の Critical）の脆弱性が公開された。一般の権限しか持たない利用者が、外部接続先の設定欄に細工した記述を登録するだけで、サーバーが自分の保管する暗号鍵・認証用の秘密・データベース接続情報といった社内の機微情報を、そのまま攻撃者のサーバーへ送り出してしまう。管理者権限は要らず、これらが漏れればインストール全体のユーザーデータと認証基盤が侵害される。修正版（0.8.4-rc1）が提供されている。
