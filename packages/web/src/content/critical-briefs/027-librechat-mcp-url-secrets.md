...
- **核心**: ユーザー由来の未検証入力(MCP URL)が、登録者の権限を確かめないままサーバーの特権文脈(`process.env`)で解釈され、設定の記述形式ひとつで権限境界が崩れた
...
本事象は Pillar 03(エージェント権限証明)の `agent-infrastructure` カテゴリに属する。中心的な**失敗 primitive は「エージェントの接続先を記述する設定値が、登録者の権限を確かめないまま、ユーザー由来の未検証入力としてサーバーの特権文脈(`process.env`)で解釈された」**点にある。secondary に `identity-auth` を併記する。
