...
NHTSA は Tesla の FSD が視界低下を扱えない問題の調査をリコール前段へ引き上げ、対象事故が過少報告された可能性を指摘した。並行して Reuters は「人間より最大 10 倍安全」という主張が非対称な比較で安全性を約 3 倍に水増ししていたと報じた。規制調査・調査報道・社内証言は事案を可視化したが、行動と申告が受理された後に作動する事後の検出にとどまる。欠けているのは、走行時・データ提出時に「判断の前提や申告した事故データ・統計が実態を反映するか」を独立検証する層であり、すべて自己申告のまま受理された。検出と事前証明は代替でなく補完である。
...
- **核心**: AI の走行判断の妥当性と、そこから導く安全性属性が、いずれも事業者の自己申告のまま独立検証から切り離されて、行動・規制開示・市場に直結した
