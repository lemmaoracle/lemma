...
- **根本原因**: エージェントは、審査対象のコードベースに含まれるファイル（バイナリ・スクリプト・ドキュメント）を、その出所と真正性を実行の前に独立検証しないまま扱った。`code_policies` バイナリが正規の `geopy` upstream に属するのか、外部から注入されたものかを区別せず、`README.md` に書かれた一文を正当な診断手順と解釈して `security.sh`（ひいては悪性バイナリ）を実行した
...
本事案は Pillar 01（来歴）の `code-provenance` カテゴリに属する。中心的な失敗 primitive は、**エージェントが、審査対象のコードベースに含まれるバイナリ・スクリプトを、その出所と真正性を実行の前に独立検証しないまま、正当な処理として実行に接続した**点にある。判断の根拠が、バイナリの見た目（囮ソースとの名前・文字列の一致）とドキュメントの体裁（`README.md` 内の推奨手順）に置かれ、コードの来歴——それが正規 upstream に属するのか、外部から注入されたのか——には置かれていなかった。secondary に、モデルが「読んでいるコード（データ）」と「従うべき指示」を区別できないまま行動した点で `ai-decision-integrity`、auto-mode の自動承認機構が同じ経路で実行へ接続する点で `agent-infrastructure` を併記する。

本事案は Brief No.099（[Agentjacking](https://lemma.frame00.com/ja/critical/briefs/099-agentjacking-sentry-mcp/)、偽のエラー報告を「解決手順」と信じてエージェントが攻撃者のコマンドを実行した）と同型である。いずれも、エージェントが取り込んだコンテンツ内の指示を、その発行元を確かめずに実行へ移した。Brief No.048（[TrapDoor](https://lemma.frame00.com/ja/critical/briefs/048-trapdoor-ai-instruction-provenance/)、AI 向け指示ファイルに見えない命令が仕込まれた）とは、AI コーディングツールが読むファイルの来歴が検証されないまま指示として作用する点で連なる。Brief No.037（[エージェント同梱設定の無検証実行](https://lemma.frame00.com/ja/critical/briefs/037-agent-config-auto-execution/)）と Brief No.095（[Amazon Q、同梱の MCP 設定が無検証で実行された](https://lemma.frame00.com/ja/critical/briefs/095-amazon-q-mcp-auto-execution/)）とは、リポジトリに同梱された「自分では出どころを選べない入力」を実行に接続する構造を共有する。Brief No.098（[BioShocking](https://lemma.frame00.com/ja/critical/briefs/098-bioshocking-agentic-browser-context/)）や Brief No.024（[不可視 Unicode による指示インジェクション](https://lemma.frame00.com/ja/critical/briefs/024-invisible-unicode-instruction-injection/)）とは、エージェントが実際に読み取る指示と、人間が目視・想定する内容との乖離が悪用された点で通じる。
