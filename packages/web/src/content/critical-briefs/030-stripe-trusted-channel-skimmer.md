...
EC セキュリティ企業 Sansec が、Stripe の正規インフラを悪用する Magecart カード窃取を公表した（Stripe 自身は無傷）。攻撃者は、ストアが既定で許可する信頼ドメイン（`api.stripe.com` / Google）をスキマーの配送と窃取データの保管に転用し、許可済みドメインゆえ CSP・egress フィルタをすり抜ける。欠けていたのは、チェックアウトで動くコードがストアの正規配置という来歴を持つかを実行前に確かめる層であり、ドメインの身元への信頼が中身の来歴の代わりに使われた。
...
> 注: 固有名・IOC は一次（研究機関・GitHub Advisory・NVD・ベンダー脅威インテリジェンス等）に基づき、各実装の対応状況は時点により異なるため最新情報を参照。Stripe / Google 自体は侵害されておらず、悪用されたのは正規 API・正規タグ管理の機能である点を誇張しない。
...
- **業界横断の論点**: 信頼ドメインを配送・保管に転用する Magecart は、CSP やドメイン allowlist というドメイン単位の防御の限界を露呈する。チェックアウトで実行されるコードの来歴を、ドメインの身元とは独立に検証するという論点が、本事案と [Brief 010](https://lemma.frame00.com/ja/critical/briefs/010-claude-code-leak-lure/) を通じて重みを増している。PCI DSS の文脈でも決済ページのスクリプト完全性管理が要求事項として強まっている

---

## 4. なぜ止まらなかったか

中心的な<strong>失敗 primitive は「CSP・ネットワーク allowlist がドメインの身元を信頼し、そのドメインが運ぶコードとデータの来歴(誰が・どの正規経路で配置したか)を検証しない」</strong>点にある。
...
本事案で露呈した検出と証明の落差(allowlist がドメインの身元を信頼し、運ばれるコードとデータの来歴を検証しない)に対して、Lemma は、実行されるコードや受け渡されるデータを、ドメインの allowlist ではなく配置者・経路・内容の来歴として実行前に独立検証可能な暗号証明として検証する設計を提示している。

- **配置来歴の証跡化**: チェックアウトで実行されるスクリプトを「誰が・どの正規経路で配置したか」として発行者署名付きで証跡化し、配送ドメインの身元とは独立に来歴を保持する
- **実行前の来歴検証**: コードが実行され・データが送出される前に、ストアの正規配置という来歴を持つことを独立検証する。来歴が成立しないスクリプトは、信頼ドメインから来ていても検出ではなく実行前の block として止まる
- **ドメイン身元と内容来歴の分離**: `api.stripe.com` が正規 API であることと、それが運ぶメタデータが正規配置であることを別問題として扱い、ドメイン単位の信頼を内容の正しさの代わりにしない

これにより、信頼ドメインが任意のコード・データを運べるという allowlist の前提の穴を、内容来歴の実行前検証で塞ぐ。「ドメインが信頼できる ≠ 中身の来歴が正しい」という来歴証明カテゴリの設計思想に立ち、ドメイン allowlist(detection)とコード来歴の事前証明は相補的に働く。信頼シグナルの転用という同型事例は [Brief 010](https://lemma.frame00.com/ja/critical/briefs/010-claude-code-leak-lure/) と合わせて参照されたい。
