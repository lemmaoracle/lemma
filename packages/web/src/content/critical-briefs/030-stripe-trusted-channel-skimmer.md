...
EC セキュリティ企業 Sansec が、決済サービス Stripe の正規インフラを悪用するカード情報窃取キャンペーンを公表した。Stripe 自身は侵害されていない。攻撃者は、オンラインストアが当然のものとして許可している信頼ドメイン（Stripe と Google）を、カード窃取コードの配送路と盗んだデータの保管先の両方に転用した。買い物客の決済時に、攻撃者の Stripe アカウント上の正規データ欄に隠したコードが組み立てられて動き、盗んだカード情報（番号・有効期限・セキュリティコード・氏名・住所・連絡先）を、同じ Stripe の正規 API 経由で攻撃者側へ送り出す。送受信先がいずれもストアの許可済みドメインのため、通常のセキュリティ検査をすり抜ける。活動は 2025-12-24 まで遡り、Google のサービスを使う変種も確認された。
