...
2026 年 6 月 4 日、EC セキュリティ企業 Sansec が、Stripe の API インフラを悪用する新たな Magecart(Web スキミング)キャンペーンを公表した。**Stripe 自身は侵害されていない。** 攻撃者は、オンラインストアが暗黙に許可している信頼ドメイン(`api.stripe.com` と `googletagmanager.com`)を、カード窃取ペイロードの**配送路**と窃取データの**保管バックエンド**の双方に転用した。スキマーは Google Tag Manager コンテナから読み込まれ、Stripe API の特定の顧客レコードのメタデータフィールドに格納された JavaScript を再構成して `new Function()` で実行。窃取したカード情報(番号・有効期限・CVV・氏名・住所・連絡先)は XOR で難読化してローカル保管後、**攻撃者の Stripe アカウントに偽の顧客レコードを作成して**格納する。ストアが Stripe ドメインを既定で許可しているため、CSP とネットワークフィルタを通過する。顧客レコードの作成は 2025-12-24 に遡り、Google Firestore を使う変種も確認された。本事案は Pillar 01(来歴証明)の `code-provenance` における、**allowlist がドメインの「身元」を信頼し、そのドメインが運ぶコードとデータの「来歴」を検証しない**構造を示し、Brief 010(信頼シグナルを配送路に転用)に連なる。
