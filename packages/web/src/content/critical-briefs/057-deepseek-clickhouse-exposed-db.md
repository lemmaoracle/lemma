...
2025 年 1 月、Wiz Research は、AI 企業 DeepSeek の ClickHouse データベースが認証なしでインターネットに公開され、公開ポート経由で誰でも到達でき、100 万行超のログ、平文のチャット履歴、API キーや秘密トークンが露出していたことを発見した。外部スキャンや責任ある開示など事後の検出は露出が成立した後にしか働かず、認証のない基盤では到達した主体が正当かを区別する手段がそもそも無い。検出と事前証明は代替でなく補完である。
