...
2025 年 1 月、セキュリティ企業 Wiz Research は、AI 企業 DeepSeek の社内データベースがパスワードなしで誰でも見られる状態でインターネットに公開されていることを発見した。公開ポート経由で誰でも到達でき、100 万行を超えるログ、暗号化されていない利用者のチャット履歴、社内の認証情報（API キーや秘密トークン）、バックエンド情報が露出していた。機微なデータ基盤へのアクセスに、相手が正当かを確かめる認証・権限検証がそもそも無く、ネットワークで到達できることがそのまま全データの取得になっていたことが核心である。Wiz は責任ある開示を行い、DeepSeek は約 30 分で是正した。
