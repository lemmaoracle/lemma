...
gap_detected: "外部からの継続的なアタックサーフェス監視と責任ある開示が働き、露出は悪用の証拠が広がる前に是正された。"
gap_missing: "「このアクセスは正当な権限に基づくか」をアクセスの前に確かめる層が無く、認証のない基盤では到達した主体が正当か否かを区別する手段がそもそも存在しなかった。"
gap_fix: "高リスク行動の前に「この主体は、このスコープのデータへの権限を正当に持つ」ことを Lemma で独立検証して、事前に防ぐ。"
---

## 1. TL;DR

2025 年 1 月、Wiz Research は、AI 企業 DeepSeek の ClickHouse データベースが認証なしでインターネットに公開され、公開ポート経由で誰でも到達でき、100 万行超のログ、平文のチャット履歴、API キーや秘密トークンが露出していたことを発見した。外部スキャンや責任ある開示など事後の検出は露出が成立した後にしか働かず、認証のない基盤では到達した主体が正当かを区別する手段がそもそも無い。
...
> 注: 固有名・CVE は一次（研究機関・GitHub Advisory・NVD 等）に基づき、各実装の対応状況は時点により異なるため最新情報を参照。本件は研究機関による責任ある開示に基づき、第三者による不正取得の有無を断定するものではない。
...
中心的な failure primitive は、**AI サービスの機微データ基盤に対し、アクセス主体の認証・権限属性が独立検証されないまま、ネットワーク到達が全取得に直結する**ことである。

[Brief 036](https://lemma.frame00.com/ja/critical/briefs/036-commonpool-training-data-pii/)（公開学習データの PII）はデータの来歴・同意の層を扱うが、本件は**稼働中の AI サービスの運用データ**が露出した点で断面が異なる。[Brief 056](https://lemma.frame00.com/ja/critical/briefs/056-mchire-paradox-recruiting-auth/)（McHire）と同じ「権限検証の不在」primitive を共有するが、McHire が推測可能な資格情報＋ IDOR だったのに対し、DeepSeek は**認証そのものが存在しない**より根源的な欠落である。AI 企業がスケールを優先する中で、データ基盤の到達制御と認可が分離されないまま公開される、というパターンを示す。
...
現状、AI サービスのデータ基盤では、到達制御（ネットワーク）と認可（誰が何にアクセスできるか）が分離されないまま運用される例が後を絶たない。事前証明（pre-execution attestation）は、データ基盤へのアクセス経路に「主体がこのスコープの権限を正当に持つ」という属性証明を前置し、到達と認可を構造的に分離する。検出（外部スキャン・開示）は被害の縮小に、事前証明（アクセス時の権限検証）は認可の独立検証に、**補完** 的に寄与する。
...
行動時点で固定された証明は、後日「このアクセスは正当だったか」を問われたときに、機微データを開示せずに独立検証できる証跡として機能する。検出（外部スキャン・開示）は被害の縮小に、事前証明（アクセス時の権限検証）は認可の独立検証に、補完的に寄与する。
