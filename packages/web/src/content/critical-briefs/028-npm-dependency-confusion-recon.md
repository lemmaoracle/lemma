...
単一の攻撃者が、実在する企業の社内向け部品名になりすました 33 個以上の悪性ソフトウェア部品を、公開のソフトウェア配布拠点（npm）に登録した。開発環境が社内部品より誤ってこの偽部品を取り込んでしまう「依存関係混乱」という手口で、`package.json` には実在しそうな社内インフラの URL まで偽装し、一見正当に見せかけていた。部品の導入時に難読化された不正プログラムが自動起動し、監視の濃い CI 環境では沈黙、それ以外では外部の指令サーバーから本体を取得して環境変数・認証情報・開発状況を収集する。現状は情報収集（偵察）に留まる設定だが、攻撃者側の切り替えひとつで認証情報窃取などの本格的な攻撃へ移れる二段構えである。セキュリティ調査を受け、該当部品群は削除された。
