...
「Alias Robotics らが、Unitree の人型・四足ロボットを Bluetooth 初期設定から乗っ取る手法「UniPwn」を公開した。全機が同じ 1 本の鍵を共有しており、合言葉を送れば認証を通過し、送り込んだプログラムを最上位権限で実行できた。鍵が全機共通ゆえ 1 台の手口が他機にも通用し、自己伝播しうる。機体ごとの identity がなく、1 つの共有秘密が機体群全体の信頼を兼ねていた。検出と、行動前に認可を独立検証する事前証明は代替でなく補完である。」
