...
セキュリティ研究者ら（解析はセキュリティ企業 Alias Robotics）が、Unitree の人型・四足ロボット（G1・H1・Go2・B2）を Bluetooth 経由の初期設定から乗っ取れる手法「UniPwn」を公開した。核心は、すべての機体が同じ 1 本の鍵を共有していたことにある。攻撃者は近づいて決まった合言葉を送るだけで認証を通過でき、続いて送り込んだプログラムをロボットが確かめずに最上位の権限で実行してしまう。鍵が全機共通なので、1 台を乗っ取る手口がそのまま他の機体にも通用し、近くの機体へ次々と自己伝播しうる。1 台分の鍵を取り出せば全機の設定データも解読できた。研究者は事前にメーカーへ通報したが対応は限定的だったと報告されている。
