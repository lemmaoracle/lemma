...
ロボットを Wi-Fi につなぐとき、スマホから Bluetooth 越しにネットワーク名とパスワードを渡す——その「初期設定」のやり取りが、機体を乗っ取る入口になりうる。2025 年 9 月 20 日、研究者 Andreas Makris と Kevin Finisterre（解析は Alias Robotics）は、Unitree の人型・四足ロボット（G1・H1・Go2・B2）の BLE（Bluetooth Low Energy）初期設定インターフェースを悪用する手法「**UniPwn**」を公開した。中核は、**全機が同一のハードコードされた暗号鍵を共有していた**ことにある。攻撃者は、文字列 `unitree` をその共有鍵で暗号化して送るだけで認証を通過でき、続いて Wi-Fi の SSID・パスワードに偽装して送り込んだコードを、ロボットが検証なしに root 権限で実行してしまう。鍵は全機共通なので、必要なのは BLE の近接と、ソフトウェアから抽出できる「みんなが同じ」秘密だけ。1 台を取れれば、同じ鍵を持つ他の機体も同様に取れ、近接機へ自己伝播しうる（wormable）。設定ファイルを守る暗号も全機同一の 128bit 鍵で、1 台を解読すれば全機を解読できた。本 Brief は本事案を Pillar 03（エージェント権限証明）の観点から、**実体エージェントに per-device の identity（機体ごとに固有で、独立検証できる本人性）が存在せず、共有された 1 つの秘密が機体群全体の信頼を兼ねていた**構造として分析する。Brief 068（命令の送り手の権限を物理動作の前に検証しない産業用ロボット）の姉妹事例である。
...
- **核心**: 実体エージェントの「本人性」が機体ごとに固有でなく、共有された 1 つの秘密に集約されていた。per-device の identity と、その独立検証が存在しなかった
...
## 3. 事象連鎖：共有された 1 つの秘密が、機体群の本人性を兼ねる
...
本事案は Pillar 03（エージェント権限証明）に属する。中心的な失敗 primitive は、**実体エージェントに per-device の identity（機体ごとに固有で独立検証できる本人性）がなく、共有された 1 つのハードコード秘密が機体群全体の認証を兼ねていた**点にある。「全機が同じ鍵を持つ」とは、1 台の信頼と機体群の信頼が分離されていないことを意味し、1 台の侵害がそのまま fleet の侵害になる。さらに初期設定経路は、その共有秘密で「認証済み」とされた相手の入力を検証なく root 実行した。primary に `identity-auth`（機体ごとの本人性と命令の認可）、secondary に `agent-infrastructure`（ロボットという実体基盤）と `attribute-proof-bypass`（本人性属性の独立検証不在）を併記する。

Brief 068（Universal Robots PolyScope、命令の送り手の権限を物理動作の前に検証しない産業用ロボット）の姉妹事例であり、対エージェントの物理クラスタを成す。068 が「命令の認可検証が経路上に存在しない」事例だったのに対し、本事案は **認証は存在するが、それが全機共通の単一秘密で、機体ごとの固有性・独立検証を欠く**点が異なる。Brief 033（暗黙に信頼された一点の侵害が、保存資格情報ごと全体へ横展開した）とは、信頼された一点（共有鍵）の突破が機体群全体へ連鎖する構造で同根。Brief 006（認証情報の失効属性が独立検証されず削除後も有効だった）とは、認証情報の属性（ここでは「機体ごとに固有か」）が独立検証されない点で、Brief 003（Host ヘッダー操作で認証が回避された）・Brief 025（設定→コマンド実行が広範な RCE 経路になった）とは、認証・設定経路の入力が検証なく特権側に通る点で連なる。
...
- **業界横断の論点**: 家庭・工場・公共空間に入る自律機が増える中、実体エージェントの本人性・命令認可をどう機体ごとに独立検証するかが安全要件として共有されつつある。Brief 068 と合わせ、ソフトウェア／物理を問わずエージェントの権限証明が必要という認識が広がっている
...
- **行動ごとのスコープ認可（proof-as-auth）**: 初期設定・運転・更新のような特権操作を、機体固有の identity と付与者の認可に対して行為の時点で独立検証する。共有秘密の提示では満たせない、操作ごとの証明に置き換える（Brief 068 の物理命令認可と接続）
