...
これにより、行為の時点で固定された証明が、「この機体は固有に正当か、この操作は認可の範囲内か」を、事後のトラフィック解析に依存せず独立検証可能なトレイルとして機能させる。検出（事後の解析・更新・分離）は露出の是正に、事前証明（機体固有の本人性・認可の独立検証）は実体エージェントの信頼確立に、それぞれ相補的に働く。設計と適用範囲は、[Pillar 03 — エージェント権限証明](https://lemma.frame00.com/ja/pillars/#authority) および [Trust402](https://lemma.frame00.com/ja/trust402/) を参照のこと。
