...
- **核心**: 認証が「パスワードを知っている」ことのみに依存し、要求者が正規の権限者であることを証明する層がなかったため、盗まれた認証情報は正規アクセスと区別できなかった構造である。
...
本事案は Pillar 04（規制属性証明）の `identity-auth`（横断）カテゴリに属する。中心的な**失敗 primitive は「認証が『このパスワードを知っているか』という知識の確認のみに依存し、『このパスワードを入力しているのが正規の権限者かどうか』を独立に証明する層を持っていなかった」**である。
...
本事案が既存 Brief との比較で示す固有論点は**規模**と**インフラ影響**にある。Brief No.046（ServiceNow 設定ミスで認証が外れた）・No.057（DeepSeek ClickHouse 未認証公開）・No.056（McHire Paradox 採用 AI 認証欠如）はいずれも「認証ゲートが機能しなかった」事案だが、本事案は「正規のゲートに認証情報でログインされた」点が異なる。攻撃はシステムの欠陥を突いたのではなく、**認証の前提（「このパスワードを入力しているのは権限者だ」）を崩した**。

secondary カテゴリとして `attribute-proof-bypass` を併記する。HIPAA の文脈では、PHI（保護対象医療情報）へのアクセスは「正規の業務上の必要性を持つ権限者」に限定されるべきだが、PHI アクセスの権限属性を独立検証する層が不在であり、侵害者が正規アクセスと区別できない状態でその制約は機能しなかった。
...
検出（インシデント検知・封じ込め）と事前証明（access の権限属性の独立検証）は代替ではなく補完であり、事後の検知を否定せずに、認証情報の一致だけに依存しないゲートを設計の前段に置く。設計と適用範囲は [Pillar 04 — 規制属性証明](https://lemma.frame00.com/ja/pillars/regulatory-attribute-proof/) および [Seal](https://lemma.frame00.com/ja/seal/) を参照のこと。
