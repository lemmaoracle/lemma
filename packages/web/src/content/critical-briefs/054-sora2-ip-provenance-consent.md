...
2025 年 10 月、OpenAI は動画生成 AI「Sora 2」を、権利者が拒否（opt-out）しない限り著作権キャラを生成できる方針で公開した。許可を先に取らず嫌なら後から拒否させる順番の逆転により、One Piece・鬼滅・ポケモン等を含む動画が拡散し、約 3 日で事前許諾（opt-in）へ転換、CODA・日本政府も是正を要請した。事後の異議申立てや出力フィルタでは、生成の前に素材の権利・同意・来歴が固定されていない構造には届かない。事後の検出と、行動前に出所・認可を独立検証する事前証明は代替でなく補完である。
