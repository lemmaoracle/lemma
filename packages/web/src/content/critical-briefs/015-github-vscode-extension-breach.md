...
2026 年 5 月、ある攻撃グループが、毒を仕込んだ開発ツール（人気のある正規 VS Code 拡張に細工した偽版）を公式マーケットにわずか 18 分間だけ公開し、その間にインストールした GitHub 従業員の端末から、パスワード管理ツールの保管庫や各種クラウド・開発サービスの認証情報を抜き取った。攻撃者はこれを使って GitHub の社内リポジトリ約 3,800 件を複製・窃取し、犯罪フォーラムで売りに出した。GitHub は翌 19 日に検知して即日対応し、顧客のリポジトリやアカウントには影響していないと公表している。開発者が日々何気なく信頼している正規の配布経路が、成果物の安全性までは保証しない、という構図である。
