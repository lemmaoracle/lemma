...
2026 年 2 月、自律型を名乗る攻撃者 hackerbot-claw が複数の人気 OSS の CI/CD を悪用し、初の記録された AI 対 AI 攻撃を仕掛けた。攻撃者は、AI コーディングエージェントが行動指針として読み込む `CLAUDE.md` を、防御側 AI を操る誘導文に書き換えた。今回は Claude が injection を拒否したが、検知はモデル次第で別のエージェントでは素通りし得る。外部の指示を出所や改ざんを確かめず取り込む構造は残る。事後の検出と、行動前に出所/認可を独立検証する事前証明は代替でなく補完である。
