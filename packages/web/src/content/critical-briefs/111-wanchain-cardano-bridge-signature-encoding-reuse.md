...
本事案は Pillar 01（来歴）の `bridge-config-trust` カテゴリに属する。secondary に、署名が指す「行動そのもの（払い出し額・宛先）」の一意性という面で `data-provenance` を、有効な署名が本来認可した範囲を越えて通用した面で `identity-auth`（認証・認可）を併記する。
