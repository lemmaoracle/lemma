...
家庭の掃除ロボットは、カメラ・マイク・間取り図をクラウド経由でやり取りする——その基盤が「認証は済んでいるか」だけを見て「この利用者は、この 1 台に対して権限を持つか」を確かめないと、認証済みの誰か 1 人が他人の機体すべてに届いてしまう。2026 年 2 月、研究者 Sammy Azdoufal が、自分の DJI ROMO ロボット掃除機を PS5 コントローラーで動かそうと（AI コーディングツールを使って）アプリの通信プロトコルを解析していたところ、自作アプリを DJI のクラウド基盤に接続した瞬間、**約 7,000 台（24 か国以上）の他人の ROMO が応答した**。原因は、機体とサーバー間の MQTT 通信を仲介するブローカーに **ACL（アクセス制御リスト）が無く、認証済みのクライアント 1 つがあらゆる機体のトピックを購読できた**こと。これにより、ライブのカメラ映像・マイク音声・間取りデータに、本来は無関係な利用者からアクセスできる状態が生じた。研究者は The Verge 経由で DJI に通報し、DJI は 2 月 8 日・10 日の更新で修正、$30,000 の報奨金を支払った。本 Brief は本事案を Pillar 03（エージェント権限証明）の観点から、**クラウド基盤が「認証済みか」と「この機体に対する認可があるか」を分離せず、機体ごとの認可を独立検証していなかった**構造として分析する。Brief 070（機体ごとの identity が無い人型・四足ロボット）の消費者・スマートホーム版である。
...
- **根本原因**: 機体とサーバー間の MQTT 通信を仲介するブローカーに ACL（アクセス制御リスト）が無く、**認証を通った 1 つのクライアントが、特定機体に紐づかずあらゆるトピックを購読できた**。バックエンドの権限検証（どの利用者がどの機体に対して権限を持つか）が欠けていた
...
- **核心**: 認証（authenticated か）と認可（この機体に対して権限を持つか）が分離されておらず、機体ごとの認可をブローカーが独立検証していなかった。「認証済み＝全機にアクセスしてよい」になっていた
...
## 3. 事象連鎖：1 つの認証が、機体群すべての映像・音声に届く
...
本事案は Pillar 03（エージェント権限証明）に属する。中心的な失敗 primitive は、**クラウド基盤が「認証済みか」と「この機体に対する認可があるか」を分離せず、機体ごとの認可を独立検証していなかった**点にある。MQTT ブローカーに ACL が無いとは、「認証を通った者は、どの機体のトピックでも購読できる」を意味し、到達（reachability）がそのまま全機のセンサーへのアクセスになっていた。primary に `identity-auth`（機体ごとの認可）、secondary に `agent-infrastructure`（クラウド／IoT 基盤）と `attribute-proof-bypass`（アクセス主体の権限属性の独立検証不在）を併記する。

Brief 070（Unitree、全機が同一のハードコード鍵を共有し、機体ごとの identity が無い）の姉妹事例であり、対エージェントの実体クラスタを成す。070 が「機体ごとの本人性（identity）が無い」事例だったのに対し、本事案は **認証は機能していたが、機体ごとの認可（authorization）が分離されていなかった**点が異なる——両者は「per-device の identity と認可」という同じ問いの裏表である。Brief 057（認証のない AI バックエンドで、到達がそのまま全取得になった）とは、到達と認可が分離されない構造でほぼ同型。Brief 006（認証情報の失効属性が独立検証されなかった）とは、アクセス主体の属性が独立検証されない点で、Brief 052（年齢確認のために預けた生 ID が第三者保管で流出した）とは、消費者の機微データ（ここではカメラ・マイク・間取り）が分離・保護されないまま露出面に転じる点で連なる。
...
- **業界横断の論点**: 家庭・生活空間に入る実体エージェント（掃除ロボット、見守り機器等）が増える中、Brief 070（機体 identity）と合わせ、per-device の identity と認可をどう独立検証するかが、プライバシーと安全の要件として共有されつつある
...
- **到達と認可の分離**: 「基盤に到達できた＝アクセスしてよい」という前提を断ち、reachability と authorization を分離する（Brief 057 の到達＝全取得構造と接続）
- **per-device identity との一体運用**: 機体ごとの固有の本人性（Brief 070）と、その機体に対する認可を一体で扱い、認証済みの 1 接続が機体群全体に届く経路を断つ
