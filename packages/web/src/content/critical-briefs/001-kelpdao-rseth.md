...
KelpDAO / rsETH で、LayerZero Labs の内部 RPC が改ざんされ、DVN が改ざんデータに正規署名を下し、116,500 rsETH（約 460 億円相当）が不正アンロックされた。盗まれたのは署名鍵ではなく、承認が参照する観測層の入力である。署名も手続きも正規だったため、署名鍵の異常使用を見る事後の検出は発火しにくい。観測層の入力を承認前に独立検証する層が欠けていた事案であり、事後の検出と、行動前に出所を独立検証する事前証明は代替でなく補完である。
