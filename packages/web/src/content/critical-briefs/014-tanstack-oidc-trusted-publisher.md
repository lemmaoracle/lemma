...
- **署名から来歴へ**: 来歴の根拠を publisher アイデンティティの署名から、ソース・ビルド入力・経路に紐づくビルド来歴の暗号証明へ移す。
- **実行前の検証**: 受信側(CI/CD・開発者環境)が取り込み前に proof を検証し、不整合なら署名が形式上有効でも reject する。
- **乗っ取り耐性**: OIDC アイデンティティが実行時に乗っ取られても、ビルド来歴の proof が別系統で「正規のビルド経路から生成された / されていない」を告げる。
- **既存層との両立**: 署名・trusted publisher を置き換えず、その上に成果物来歴の独立検証を重ねる。
