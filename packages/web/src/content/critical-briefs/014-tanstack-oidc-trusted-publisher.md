...
2026 年 5 月 11 日のわずか 6 分間に、広く使われている JavaScript ライブラリ群 `@tanstack/*` の 42 パッケージへ、合計 84 件の悪性バージョンが公開された。攻撃者は公開用の認証情報を盗んだのではなく、TanStack 自身の正規の自動公開の仕組みをビルド実行中に乗っ取り、本物の発行元として署名された悪性パッケージを、正規の配布経路からそのまま配った。導入した環境ではクラウドや社内システムの認証情報が抜き取られ、攻撃者が用意した削除トリガーで利用者のホームディレクトリが丸ごと消されるケースもあった。署名は本物でも中身は差し替えられていた、という、署名検証が当然のように通ってしまう事例である。
