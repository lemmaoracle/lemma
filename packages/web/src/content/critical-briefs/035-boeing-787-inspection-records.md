...
Boeing は、一部の 787 Dreamliner で主翼-胴体接合部の必須安全検査が記録上は「完了」とされながら実際には実施されていなかったことを FAA へ自主報告した。記録は整い書類監査やシステム上のチェックは通過していたため、乖離は事後の調査で初めて判明した。欠けていたのは「この検査記録は実際の検査行為に裏づけられているか」という記録生成時点の独立検証であり、実施を伴わない記録が「検査済み」として下流へ通用した。記録があることと、その行為が実際に行われたことは別である。
...
- **業界横断の論点**: 製造・安全・規制の各領域で、「記録の存在」を「実施・適合の証明」と取り違える構造は繰り返し顕在化している([Brief 019](https://lemma.frame00.com/ja/critical/briefs/019-construction-engineer-qualification-fraud/)/020/021 参照)。電子記録・トレーサビリティの整備が進むほど、記録の整合性と、記録が示す行為の実在性を、どう独立に証明するかが論点になる。高信頼領域(航空・基幹インフラ・医療機器等)では、属性を行為の来歴に結びつける仕組みの必要性が高い
...
本事案で露呈した構造(規制・安全属性が、実施の来歴なしに記録の存在だけで通用する)に対して、Lemma は、属性(検査済み・適合)を、記録の有無ではなく「その属性を生む行為が実際に行われた来歴」の独立検証可能な証明として扱う設計を提示している。

- **記録から行為への反転**: 「検査済み・適合」を記録の有無ではなく「その属性を生む行為が実際に行われた来歴」の独立検証として扱う。
- **実施の来歴の証明**: 誰が・いつ・実際に検査したかの来歴に属性を結びつけ、記録生成の時点で実施を伴うことを proof として要求する。
- **下流進行前の事前 block**: 記録が形式上整っていても、実施の来歴を伴う proof が成立しなければ属性を確立させず、出荷・運用への進行を事前に reject する。
- **監査との補完**: 記録の監査(揃っているか)と実施の事前証明(実際の行為に裏づけられているか)を別系統として併置する。

実施の来歴を伴う proof が成立しなければ属性は確立されず、事後監査は事前の証明によって補完される。[Brief 019](https://lemma.frame00.com/ja/critical/briefs/019-construction-engineer-qualification-fraud/)/020/021 と合わせ、「記録の存在 ≠ 実態の証明」の系譜として参照されたい。
