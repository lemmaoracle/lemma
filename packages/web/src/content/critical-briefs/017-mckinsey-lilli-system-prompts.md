...
2026 年 2 月、セキュリティ企業 CodeWall の自律型 AI エージェントが、検証許可の下で（模擬）、McKinsey の社内向け生成 AI「Lilli」に対し、認証情報も内部知識もない状態から 2 時間足らずで本番データベースの読み書きまで到達した。最も重大だったのは、Lilli の振る舞いを決める 95 件の指示文（回答の仕方やガードレール、出典の示し方を統治する設定）がすべて書き換え可能だった点である。これを悪用されれば、同社従業員の 72% が日常的に使うチャットボットの回答を、気づかれないまま改ざんできた。実害のあった攻撃ではなく責任ある開示を伴う検証だが、AI の判断を統治する設定と出力が本物かどうかを独立に確かめる仕組みが無い、という弱点を露わにした。
