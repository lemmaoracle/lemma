...
2026 年 5 月 11 日、Trend Micro(TrendAI Research)が、ラテンアメリカの政府機関と金融機関を標的に **agentic AI が侵入オペレーションを駆動した 2 つのキャンペーン**(SHADOW-AETHER-040 / SHADOW-AETHER-064)を公表した。同社は、AI エージェントが初期侵入から情報持ち出しまでを実行した最初期の観測事例と位置づけている。SHADOW-AETHER-040(スペイン語話者)は 2025 年末から活動し、2025-12-27〜2026-01-04 にメキシコの 6 政府機関を侵害、AI エージェントの支援でキルチェーン全体を実行した。SHADOW-AETHER-064(ポルトガル語話者)は 2026 年 4 月以降、ブラジルの金融機関を標的とした。決定的なのは、**AI エージェントが既製ツールに頼らず標的ごとに攻撃ツール・スクリプトを動的生成**し、既知ツールの署名に依存する従来型検出の捕捉確率を下げた点である。本事案は Pillar 03(エージェント権限証明)の `agent-runaway` における、**実行されるツール・操作が固有の署名を持たないとき、検出は何を実行してよいかの認可・来歴に置き換わらなければならない**という構造を示し、Brief 009(GTG-1002)・026(自律 AI ワーム)に連なる、実地で観測された代表事例である。
...
事前証明(pre-execution attestation)は、検出を「既知の悪性物の照合」から、「実行されようとしている操作・コードが認可され来歴を持つかの実行前検証」へと反転させる設計を採る。ツールが未知・新規生成であっても、proof が「この操作は正規に認可された来歴を持たない」と告げれば実行は事前に block される。署名ベースの検出(detection 的な「既知の悪性か」)と操作の事前証明(「これは認可・来歴のある実行か」)は代替ではなく**補完**の関係にあり、攻撃ツールが AI 生成で署名を持たない世界では後者の比重が増す(検出と事前証明の thesis は [「AI 時代のサイバー防衛に残された、最後の層」](https://lemma.frame00.com/ja/blog/detection-is-not-proof/)(Lemma、2026-05)を参照)。
...
本事案で露呈した構造的な問題(攻撃ツールが標的ごとに AI 生成され固有署名を持たないため、既知物照合に依拠する検出が後追いになる)に対して、Lemma は、検出を「既知の悪性物の照合」から「実行されようとする操作・コードの認可と来歴の実行前検証」へ反転させる設計を提示している。ツールが未知でも、操作の認可・来歴の proof が成立しなければ実行は事前に reject される。エージェントの委任と実行を証跡化する設計思想は [Pillar 03 — エージェント権限証明](https://lemma.frame00.com/ja/pillars/agent-authority-proof/)(Lemma)を参照のこと。
