...
LiteLLM, BerriAI's open-source "AI gateway" for centralizing access to AI models, carried an authentication-bypass flaw, CVE-2026-59822, in its MCP Streamable HTTP endpoint. When LiteLLM's own key validation failed, the fallback replaced that failure with an empty authorization object treated as always valid — so an attacker holding no valid key could reach connected MCP tools by sending a fabricated Authorization header. It was disclosed and patched in v1.84.0 on June 30, 2026; on September 2 CISA confirmed active exploitation and added it to the KEV catalog. **Authentication was built to fail. Past that failure there was no layer that denied — the absence of verification was itself treated as permission.**
...
<strong>A failed check and no check at all produced the same outcome in the system's behavior.</strong> This was not a clever evasion technique — it was a flaw in the design of the fallback path itself. The fix, v1.84.0, shipped on the disclosure date, 2026-06-30. CISA nonetheless confirmed active exploitation and added the flaw to KEV roughly two months later, on 09-02: a fix existing and a fix reaching the gateways in the field are two different things.
...
The difference from after-the-fact vulnerability scanning and KEV listing is here: a KEV addition confirms, after the fact, that a vulnerability was exploited. Though the fix shipped on the disclosure date, this design kept running inside MCP gateways that were never updated, and it took roughly two months before that exploitation was confirmed.
