...
On May 28–29, 2026, a single operator, using three npm maintainer accounts, published 33+ malicious packages across nine organization scopes impersonating the internal namespaces of real companies. Using the dependency-confusion technique, the packages spoofed internal names like `@cloudplatform-single-spa` and `@sber-ecom-core/sberpay-widget` (impersonating Sberbank's SberPay), and planted plausible-looking internal GitHub Enterprise, Jira, and documentation portal URLs in `package.json`. A `postinstall` hook launches an obfuscated stager that goes silent when it detects a CI environment; otherwise, it fetches a payload from C2 and collects environment variables, credentials, and development context. At present, the stager is locked to `RECON_ONLY=1` reconnaissance mode, but a server-side flag switch enables a transition to full exploitation — a two-stage design. Following feedback from Microsoft Threat Intelligence, the package set was removed. This campaign belongs to Pillar 01 (Verifiable Origin) `code-provenance` and illustrates that **package names and metadata "looking internal" are used in place of provenance during registry resolution.** It joins the supply-chain cluster of Briefs 004 / 014 / 015.
