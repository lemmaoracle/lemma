...
gap_detected: "Searchlight Cyber discovered and reported the pre-auth sandbox-escape RCE, ServiceNow shipped patches, and Defused detected and warned of in-the-wild exploitation."
gap_missing: "Whether execution is allowed is decided by the surface signal 'could it reach an unauthenticated endpoint / is it inside the sandbox,' and because per-action authorization is not independently verified, exploitation reached the same code execution even by a path different from the public PoC."
gap_fix: "Before privileged execution begins, require — independent of the reach path or the sandbox boundary — a verifiable proof that the action is authorized, and cut the chain by which a single point of reach escalates to admin creation, MID Server reach, and full takeover."
---
...
With the same vendor as Brief No.046 ([ServiceNow unauthenticated API](/critical/briefs/046-servicenow-unauthenticated-api/), where a single setting removed authentication and customer instances were queried while unauthenticated), it repeats the structure in which "unauthenticated reach becomes a privileged operation directly" (it is independent as an incident — a different CVE, a different endpoint, a sandbox escape). It is of the same type as Brief No.088 ([Kestra](/critical/briefs/088-kestra-auth-filter-bypass-rce/), where authentication fell off at the end of a path and code ran as root while unauthenticated) and Brief No.003 ([Starlette / BadHost](/critical/briefs/003-starlette-badhost/), where Host-header manipulation bypassed authentication), in the structure where a surface judgment of authentication/authorization connects directly to execution privilege. It connects with Brief No.094 ([Cursor / DuneSlide](/critical/briefs/094-cursor-duneslide-sandbox-escape/), where a single planted instruction escaped the sandbox to arbitrary code execution) on the sandbox-escape surface, and with Brief No.033 ([F5 BIG-IP](/critical/briefs/033-f5-bigip-edge-pivot/), where the compromise of a single device chained to the whole domain) on the surface where a single point of reach escalates to full takeover. The shared primitive is the same: **reachability and surface signals are independent of per-action authorization.**
...
- **BleepingComputer**: “Critical ServiceNow code execution flaw now exploited in attacks” (2026-07-20; Defused's confirmation of in-the-wild exploitation, the different-path sandbox escape, ServiceNow's statement) — <https://www.bleepingcomputer.com/news/security/critical-servicenow-code-execution-flaw-now-exploited-in-attacks/>
