...
In May–June 2026, both research and real-world incidents showed that a developer's ordinary act of "opening" a repository in an AI coding tool (Claude Code, Cursor, Gemini CLI, and others) can become the trigger for credential theft. The cause is that AI agents automatically execute configuration files bundled in a project without checking what is inside them. On the research side, Adversa AI disclosed SymJack (2026-05-26) and TrustFall (disclosed 2026-05). SymJack uses symbolic-link spoofing to display an action as "copying a video" while actually overwriting the agent's own configuration file, so attacker code runs at the next launch; it was confirmed on Claude Code, Gemini CLI, Cursor, GitHub Copilot CLI, Grok Build, and OpenAI Codex CLI. TrustFall is a technique in which, the moment a folder-trust prompt is approved, a project-defined MCP server auto-starts and reaches one-click remote code execution. On the real-world side, the self-propagating malware Miasma planted `.claude/settings.json`, `.gemini/settings.json`, and Cursor configuration in repositories and executed a credential-stealing payload the moment a developer opened the project in Claude Code, Gemini CLI, or Cursor (its third wave) — leading GitHub, on 2026-06-05, to disable 73 repositories across four Microsoft organizations (Azure, Azure-Samples, Microsoft, MicrosoftDocs). This Brief addresses the structure in which, when an agent executes bundled config, the layer that independently verifies that config's authority and provenance before the action is missing. It analyzes the research disclosures and the internationally reported real-world incident structurally, from the standpoint of independent verification of agent authority. The role of the detection side and vendors is drawn as a division of labor, not a denial.
...
## 3. Attack path: how the agent comes to execute bundled config
