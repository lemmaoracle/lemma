...
On June 2, 2026, CVE-2026-32625 (CVSS 9.6, Critical) was published against LibreChat, a multi-provider AI chat platform. In versions ≤0.8.3, the MCP server integration resolves `${VAR}` placeholders inside user-supplied MCP server URLs against the server's own `process.env` during Zod schema validation. A low-privilege authenticated user simply registers an attacker-controlled domain URL embedding `${CREDS_KEY}` or `${MONGO_URI}` as an MCP server — and the LibreChat server sends encryption keys, JWT secrets, and DB connection strings to the attacker's server encoded in the request URL. No admin privileges are required, and the installation's entire cryptographic foundation and authentication material is compromised. The fix is 0.8.4-rc1. This incident belongs to Pillar 03 (Agent Authority Proof) `agent-infrastructure` and illustrates that **the configuration values describing where an agent connects to are themselves unverified inputs interpreted in a privileged context.** Together with Brief 003 (MCP server authentication bypass), it forms a trust-boundary issue in the agent infrastructure layer.
