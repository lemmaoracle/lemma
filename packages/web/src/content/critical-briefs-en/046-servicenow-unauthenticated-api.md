...
The central failure primitive is that **a request to the endpoint was accepted by relying on a configuration-level assumption — "authentication is surely being enforced" — rather than making the requester prove their authorization per action.** We note `agent-infrastructure` (the API as an infrastructure layer) and `attribute-proof-bypass` (the "authenticated" attribute presumed without authenticity verification) as secondary categories.
...
It is the same family as [Brief 006](/critical/briefs/006-google-api-key-revocation-lag/) (the revocation attribute of a Google API key was not independently verified, and the key stayed valid even after deletion): a structure in which the state of authentication/authorization is taken as a premise of trust but is not independently verified. It shares its root with [Brief 033](/critical/briefs/033-f5-bigip-edge-pivot/) (positional trust of an edge device plus stored credentials let lateral movement be accepted) in that "a once-established trust premise carries over without being bound to the scope of the action," and with [Brief 029](/critical/briefs/029-github-dev-oauth-token/) (an OAuth token valid for all repositories rather than scoped to the target) in that authorization is not bound to the scope of the action. This incident is a real-world case where that primitive surfaced at the **API boundary of enterprise SaaS** and spilled over into the queryability of entire customer instances.
