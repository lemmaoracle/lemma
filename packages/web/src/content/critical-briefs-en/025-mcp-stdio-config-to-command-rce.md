---
brief_no: 25
title: "MCP の設定→コマンド実行という設計と、供給網規模の RCE"
title_en: "MCP Design: Config-to-Command Execution and Supply-Chain-Scale RCE"
...
In April 2026, security firm OX Security disclosed that Anthropic's Model Context Protocol (MCP) official SDK contains a design-level issue in which configuration flows directly into command execution, enabling arbitrary command execution (RCE) on vulnerable MCP implementations. The issue is not a single-language implementation bug but is described as **inherent in the reference SDK's design across supported languages** — Python, TypeScript, Java, Rust, and others. The reported impact surface spans over 150 million cumulative downloads, more than 7,000 publicly listed servers, and up to approximately 200,000 instances. Multiple independently reported CVEs trace back to the same core issue (MCP SDK command injection CVE-2026-30623, among others), and the CSA (Cloud Security Alliance) consolidated the picture under "MCP by Design: RCE Across the AI Agent Ecosystem." Anthropic did not alter the core architecture, and the behavior was reported to be characterized as "expected." This Brief examines the structure in which agent-infrastructure trust boundaries — where do configuration and input stop being merely accepted and start being treated as authorized execution? — propagate through the entire supply chain without independent verification at the reference-implementation design level.
