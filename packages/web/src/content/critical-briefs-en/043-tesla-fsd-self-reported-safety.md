...
Neither the claim that self-driving is "safer than humans" nor the data on how many crashes actually occurred had been independently verified — both rested on the maker's own reporting. On March 18, 2026, the U.S. NHTSA escalated its investigation into Tesla's Full Self-Driving (FSD) inability to handle reduced visibility (sun glare, fog, dust) to an Engineering Analysis (EA26002, ~3.2 million vehicles), the stage before a recall, noting that "degradation detection" failed to warn until just before a collision and that internal data-labeling constraints may have led to under-reporting of the crashes in scope. In parallel, on May 28, 2026, Reuters reported that Tesla's "FSD is up to 10× safer than humans" claim rested on a flawed comparison that inflated safety by roughly 3×. This Brief analyzes — from the standpoint of a division of labor with detection — the structure in which both an AI's driving decisions and the safety attributes presented to regulators and markets (crash rate, the "safer" claim) remain self-reported by the operator and are not independently verified.
