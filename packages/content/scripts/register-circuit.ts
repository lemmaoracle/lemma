...
const uploadToPinata = (filePath: string, fileName: string): Promise<string> => {
  ...
  formData.append("pinataMetadata", JSON.stringify({
    name: fileName,
    keyvalues: { project: "lemma-content", circuit: CIRCUIT_ID },
  }));
...
const buildCircuitMeta = (wasmUrl: string, zkeyUrl: string): CircuitMeta => ({
  ...
  schema: SCHEMA,
  description: "Generic content commitment — proves knowledge of Poseidon hash of arbitrary file bytes (image, video, CSV, code). CID used as application-layer identifier.",
  inputs: ["fileHash"],
...
const requireEnv = (): Promise<void> =>
  LEMMA_API_KEY && PINATA_API_KEY && PINATA_SECRET_API_KEY
    ? Promise.resolve()
    : Promise.reject(new Error("Missing env vars. Set LEMMA_API_KEY, PINATA_API_KEY, PINATA_SECRET_API_KEY in packages/content/.env"));

const requireArtifacts = (): Promise<void> =>
  fs.existsSync(WASM_PATH) && fs.existsSync(ZKEY_PATH)
    ? Promise.resolve()
    : Promise.reject(new Error("Compiled circuit not found. Run `npm run build` in packages/content/circuits first."));
...
const main = async (): Promise<void> => {
  console.log("📦 Registering content-commitment-v1 circuit...");
  await requireEnv();
  await requireArtifacts();

  console.log("1. Uploading artifacts to IPFS (Pinata)...");
  const [wasmUrl, zkeyUrl] = await Promise.all([
    uploadToPinata(WASM_PATH, "content-commitment.wasm"),
    uploadToPinata(ZKEY_PATH, "content-commitment_final.zkey"),
  ]);
