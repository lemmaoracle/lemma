...
/**
 * Register forex-average-v1 circuit with artifacts on workers API.
 *
 * Pipeline:
 ...
 *   PINATA_API_KEY=... PINATA_SECRET_API_KEY=... LEMMA_API_KEY=... npx tsx scripts/register-forex-average-v1.ts
 *
 * Or source from blog-article .env (has Pinata keys)
 */
...
const CIRCUIT_ID = "forex-average-v1";
const SCHEMA = "canonical-sort-v1";

// Artifact paths (relative to monorepo root)
const MONOREPO = path.resolve("..", "..");
const CIRCUITS_DIR = path.join(MONOREPO, "packages/data-commitment/circuits");
const WASM = path.join(CIRCUITS_DIR, "build/forex-average-v1_js/forex-average-v1.wasm");
const ZKEY = path.join(CIRCUITS_DIR, "build/forex-average-v1_0000.zkey");
const VKEY = path.join(CIRCUITS_DIR, "build/forex-average-v1_vkey.json");
...
async function uploadToPinata(filePath: string, fileName: string): Promise<string> {
  ...
  formData.append("pinataMetadata", JSON.stringify({
    name: fileName,
    keyvalues: { project: "lemma-forex-average", circuit: CIRCUIT_ID },
  }));
...
async function main() {
  ...
  const [wasmUrl, zkeyUrl, vkeyUrl] = await Promise.all([
    uploadToPinata(WASM, "forex-average-v1.wasm"),
    uploadToPinata(ZKEY, "forex-average-v1_0000.zkey"),
    uploadToPinata(VKEY, "forex-average-v1_vkey.json"),
  ]);
  ...
  const meta: CircuitMeta = {
    ...
    schema: SCHEMA,
    description:
      "Forex composite average circuit. Proves that averageRate = (rateA + rateB) / 2 where rateA and rateB are committed in their respective source Merkle trees (data-commitment-v1). Rates are scaled by 10^8. Per-currency proof — pathHash is a runtime public input, so the same circuit handles any currency without recompilation.",
    inputs: ["sourceRootA", "sourceRootB", "randomnessA", "randomnessB", "pathHash", "averageRate"],
