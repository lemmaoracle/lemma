...
### Requirement: Commitment binds request provenance and response body
...
- `response`: object with `body` (the parsed JSON response)
...
- **THEN** the commitment path-value pairs include `$["request"]["url"]`, `$["request"]["fetchedAt"]`, `$["request"]["date"]`, and leaves under `$["response"]["body"]` for the response fields
...
- `response`: `{ body, canonical }` where `body` is the parsed JSON and `canonical` is the `canonical-sort-v1` string of `body` only
- `commitment`: the `commitDeep` result for the request/response envelope

#### Scenario: Canonical covers body only
- **WHEN** the upstream body is `{ "b": 2, "a": 1 }`
