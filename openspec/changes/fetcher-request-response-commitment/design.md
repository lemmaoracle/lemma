...
1. **Commitment envelope**
   - Commit `commitDeep({ request: { url, fetchedAt, date }, response: { body } }, { maxDepth })`
   ...
   - Rationale: groups provenance under `request` and payload under `response`; date leaf enables day-scoped proofs without parsing ms in-circuit
...
2. **Return shape**
   ...
   ```
   - `canonical` is still `canonicalSort(body)` only — not of the full envelope
   - Rationale: matches caller mental model; canonical remains a pure function of body

3. **Feeds adaptation (minimal)**
   - Update `FetchResult` field access: `data` → `response.body`, `source` → `request.url`, `fetchedAt` → `request.fetchedAt`
...
- [More leaves per commit] → Path prefix `$["response"]["body"]...` plus three request leaves; `maxDepth` must still cover leaf count (existing callers already pass 16)
...
3. External `/fetch` clients (if any) switch to `response.body` / `request.*`
