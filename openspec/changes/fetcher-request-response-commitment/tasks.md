...
- [x] 1.2 Change `fetchAndCommit` to capture `fetchedAt` once, derive UTC `date`, commit `{ request, response: { data } }`, return new envelope
...
- [x] 2.2 Update forex / forex-er-api worker consumers to read `response.data`
- [x] 2.3 Update `pipeline.ts` and `cli.ts` field access (`data` / `source` / `fetchedAt`)
- [x] 2.4 Rename commitment/return field `response.body` → `response.data`
