# fetcher-request-binding Specification

## Purpose

`@lemmaoracle/fetcher` binds upstream URL and fetch time (UTC ms + UTC calendar date) into the same `data-commitment-v1` Merkle tree as the response payload, and exposes a `request` / `response` result envelope from `fetchAndCommit`.
